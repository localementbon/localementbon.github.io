...
(function() {
	(function() {
		...
		fileApi =
			...
			'URL' in window &&
			!!$_ci16mbjkmcwk2b.createObjectURL;
		phone =
		...
		var fix = function(originalEvent, data) {
			...
			if ($_cvvl9iajkmcwk25.experimentalShadowDom) {
				event$$1.target = getTargetFromShadowDom(
					originalEvent,
					event$$1.target
				);
			}
		...
		var bindOnReady = function(win, callback, eventUtils) {
			...
			if (
				...
				!($_cvvl9iajkmcwk25.ie && $_cvvl9iajkmcwk25.ie < 11)
			) {
				if (isDocReady()) {
					readyHandler();
				} else {
		...
		var EventUtils = function() {
			...
			self$$1.bind = function(target, names, callback, scope) {
				...
				while (i--) {
					...
					if (
						!hasFocusIn &&
						(name$$1 === 'focusin' || name$$1 === 'focusout')
					) {
						capture = true;
		...
		Sizzle.attr = function(elem, name$$1) {
			...
			var fn = Expr.attrHandle[name$$1.toLowerCase()],
				val =
					fn && hasOwn.call(Expr.attrHandle, name$$1.toLowerCase())
						? fn(elem, name$$1, !documentIsHTML)
						: undefined;
			return val !== undefined
				? val
				: support.attributes || !documentIsHTML
				? elem.getAttribute(name$$1)
				: (val = elem.getAttributeNode(name$$1)) && val.specified
				? val.value
				: null;
		};
		...
		var skipUniques = $_4ujg4tljkmcwkbk.makeMap(
			'children,contents,next,prev'
		);
		...
		var numericCssMap = $_4ujg4tljkmcwkbk.makeMap(
			'fillOpacity fontWeight lineHeight opacity orphans widows zIndex zoom',
			' '
		);
		var booleanMap = $_4ujg4tljkmcwkbk.makeMap(
		...
		DomQuery.fn = DomQuery.prototype = {
			...
			add: function(items, sort) {
				...
				if (sort !== false) {
					nodes = DomQuery.unique(
						self$$1.toArray().concat(DomQuery.makeArray(items))
					);
						...
						if (elm.ownerDocument.defaultView) {
							try {
								return elm.ownerDocument.defaultView
									...
									.getPropertyValue(dashed(name$$1));
							} catch (ex) {
								return undefined;
							}
			...
			text: function(value) {
				...
				return self$$1[0]
					? self$$1[0].innerText || self$$1[0].textContent
					: '';
			},
			append: function() {
		...
		each$2(
			{
				parent: function(node) {
					...
					return parent$$1 && parent$$1.nodeType !== 11
						? parent$$1
						: null;
				},
				...
				contents: function(node) {
					return $_4ujg4tljkmcwkbk.toArray(
		...
		var detectBrowser = function(browsers, userAgent) {
			return detect$1(browsers, userAgent).map(function(browser) {
				var version = $_2kio3msjkmcwkcv.detect(
		...
		var detect$2 = function(userAgent) {
			...
			var browser = $_6qxoukvjkmcwkd5
				.detectBrowser(browsers, userAgent)
				.fold($_ei4lq0rjkmcwkcq.unknown, $_ei4lq0rjkmcwkcq.nu);
			var os = $_6qxoukvjkmcwkd5
				.detectOs(oses, userAgent)
				.fold($_85w4ndtjkmcwkcx.unknown, $_85w4ndtjkmcwkcx.nu);
		...
		var isComment = function(element) {
			return (
				type(element) === $_c8e5uh12jkmcwke2.COMMENT ||
		...
		var transfer = function(source, destination, attrs) {
			if (
				!$_2xwdjv11jkmcwke0.isElement(source) ||
				!$_2xwdjv11jkmcwke0.isElement(destination)
			)
				return;
		...
		var inBody = function(element) {
			var dom = $_2xwdjv11jkmcwke0.isText(element)
		...
		var get$1 = function(element, property) {
			...
			var v =
				r === '' && !$_g4uup916jkmcwkfg.inBody(element)
					? getUnsafeProperty(dom, property)
					: r;
			return v === null ? undefined : v;
		};
		var getUnsafeProperty = function(dom, property) {
			return $_crv2n17jkmcwkfl.isSupported(dom)
		...
		var remove$1 = function(element, property) {
			...
			if (
				$_29helg15jkmcwkf0.has(element, 'style') &&
				trim$2($_29helg15jkmcwkf0.get(element, 'style')) === ''
			) {
		...
		var preserve = function(element, f) {
			...
			var restore =
				oldStyles === undefined
					? $_29helg15jkmcwkf0.remove
					: $_29helg15jkmcwkf0.set;
		...
		var copy = function(source, target) {
			...
			if (
				$_crv2n17jkmcwkfl.isSupported(sourceDom) &&
				$_crv2n17jkmcwkfl.isSupported(targetDom)
			) {
		...
		var ieContains = function(e1, e2) {
			return $_ggq6431fjkmcwkgt.documentPositionContainedBy(
				e1.dom(),
				e2.dom()
			);
		};
		...
		var getTableCaptionDeltaY = function(elm) {
			if (
				browser$1.isFirefox() &&
				$_2xwdjv11jkmcwke0.name(elm) === 'table'
			) {
				return firstElement($_ca6pc118jkmcwkfs.children(elm))
					.filter(function(elm) {
						return $_2xwdjv11jkmcwke0.name(elm) === 'caption';
					})
					.bind(function(caption) {
						return firstElement(
							$_ca6pc118jkmcwkfs.nextSiblings(caption)
						).map(function(body) {
		...
		var getPos = function(body, elm, rootElm) {
			...
			if (elm) {
				if (
					...
					$_4enzit13jkmcwke5.get(
						Element$$1.fromDom(body),
						'position'
					) === 'static'
				) {
					pos = elm.getBoundingClientRect();
		...
		function StyleSheetLoader(document$$1, settings) {
			...
			var load = function(url, loadedCallback, errorCallback) {
				...
				var isOldWebKit = function() {
					var webKitChunks = navigator.userAgent.match(
						/WebKit\/(\d*)/
					);
					return !!(
		...
		var isBr = function(node) {
			return (
				$_2xwdjv11jkmcwke0.isElement(node) &&
				$_2xwdjv11jkmcwke0.name(node) === 'br'
			);
		};
		...
		var trimNode = function(dom, node) {
			...
			if ($_bjermr1rjkmcwkij.isDocument(node) === false) {
				if (
					$_bjermr1rjkmcwkij.isText(node) &&
					node.nodeValue.length > 0
				) {
					var trimmedLength = $_4ujg4tljkmcwkbk.trim(node.nodeValue)
						.length;
		...
		var setupAttrHooks = function(styles, settings, getContext) {
			...
			var keepUrlHook = {
				set: function($elm, value, name$$1) {
					...
					$elm.attr('data-mce-' + name$$1, value).attr(
						name$$1,
						value
					);
				},
				get: function($elm, name$$1) {
					return (
						$elm.attr('data-mce-' + name$$1) || $elm.attr(name$$1)
					);
				}
			};
			attrHooks = {
				style: {
					...
					get: function($elm) {
						var value =
							$elm.attr('data-mce-style') || $elm.attr('style');
		...
		function DOMUtils(doc, settings) {
			...
			var getStyle = function(elm, name$$1, computed) {
				...
				if (name$$1 === 'float') {
					name$$1 =
						$_cvvl9iajkmcwk25.ie && $_cvvl9iajkmcwk25.ie < 12
							? 'styleFloat'
							: 'cssFloat';
				}
			...
			var getOuterHTML = function(elm) {
				var node = typeof elm === 'string' ? get(elm) : elm;
				return $_bjermr1rjkmcwkij.isElement(node)
					? node.outerHTML
					: DomQuery('<div></div>')
			...
			var split = function(parentElm, splitElm, replacementElm) {
				...
				if (parentElm && splitElm) {
					...
					pa.insertBefore(
						$_br9sxm1pjkmcwki4.trimNode(self$$1, bef),
						parentElm
					);
					if (replacementElm) {
					...
					pa.insertBefore(
						$_br9sxm1pjkmcwki4.trimNode(self$$1, aft),
						parentElm
					);
					remove(parentElm);
			...
			var bind = function(target, name$$1, func, scope) {
				...
				if (settings.collect && (target === doc || target === win)) {
					boundEvents.push([target, name$$1, func, scope]);
				}
		...
		var appendAt = function(parent, element, index) {
			$_ca6pc118jkmcwkfs.child(parent, index).fold(
				function() {
					append(parent, element);
				},
				function(v) {
		...
		var remove$3 = function(element, attr, id) {
			...
			if (nu.length > 0)
				$_29helg15jkmcwkf0.set(element, attr, nu.join(' '));
			else $_29helg15jkmcwkf0.remove(element, attr);
			return false;
		};
		...
		var add$2 = function(element, clazz) {
			...
			else $_ewhsr825jkmcwkm2.add(element, clazz);
		};
		var cleanClass = function(element) {
			var classList = $_ewhsr825jkmcwkm2.supports(element)
				? element.dom().classList
				: $_ewhsr825jkmcwkm2.get(element);
		...
		var remove$5 = function(element, clazz) {
			...
			} else $_ewhsr825jkmcwkm2.remove(element, clazz);
			cleanClass(element);
		};
		var toggle$1 = function(element, clazz) {
			return $_ewhsr825jkmcwkm2.supports(element)
				? element.dom().classList.toggle(clazz)
				: $_ewhsr825jkmcwkm2.toggle(element, clazz);
		};
		...
		var has$1 = function(element, clazz) {
			return (
				$_ewhsr825jkmcwkm2.supports(element) &&
		...
		var ancestors$1 = function(scope, selector, isRoot) {
			return $_733l9k28jkmcwkmj.ancestors(
				scope,
				function(e) {
		...
		var ancestor$1 = function(scope, selector, isRoot) {
			return $_253nd92ajkmcwkmp.ancestor(
				scope,
				function(e) {
					return $_4jv6d81gjkmcwkgv.is(e, selector);
				},
		...
		var identify = function(editor, annotationName) {
			...
			var newStart = $_ca6pc118jkmcwkfs
				.child(start, rng.startOffset)
				.getOr(start);
			var closest = $_4ki4d929jkmcwkmn.closest(
				newStart,
				selector,
				function(n) {
		...
		var isAnnotation = function(elem) {
			return (
				$_2xwdjv11jkmcwke0.isElement(elem) &&
				$_ftm4v023jkmcwklz.has(elem, annotation())
			);
		};
		var findMarkers = function(editor, uid) {
			var body = Element$$1.fromDom(editor.getBody());
			return $_2gqm0n27jkmcwkmi.descendants(
				body,
		...
		var findAll = function(editor, name) {
			var body = Element$$1.fromDom(editor.getBody());
			var markers = $_2gqm0n27jkmcwkmi.descendants(
				body,
				'[' + dataAnnotation() + '="' + name + '"]'
			);
			var directory = {};
		...
		var hasContent = function(node) {
			return (
				...
				!$_bjermr1rjkmcwkij.isBr(node.firstChild)
			);
		};
		...
		var isBeforeInline = function(pos) {
			...
			return (
				pos &&
				$_bjermr1rjkmcwkij.isText(container) &&
				container.data.charAt(pos.offset()) === $_cadvcr2pjkmcwkph.ZWSP
			);
		};
		var isAfterInline = function(pos) {
			...
			return (
				pos &&
				$_bjermr1rjkmcwkij.isText(container) &&
				container.data.charAt(pos.offset() - 1) ===
					$_cadvcr2pjkmcwkph.ZWSP
			);
		};
		...
		var endsWithCaretContainer = function(node) {
			return (
				isText$2(node) &&
				node.data[node.data.length - 1] === $_cadvcr2pjkmcwkph.ZWSP
			);
		};
		...
		var isContentEditableFalse$1 =
			$_bjermr1rjkmcwkij.isContentEditableFalse;
		var isBr$2 = $_bjermr1rjkmcwkij.isBr;
		var isText$3 = $_bjermr1rjkmcwkij.isText;
		var isInvalidTextElement = $_bjermr1rjkmcwkij.matchNodeNames(
			'script style textarea'
		);
		var isAtomicInline = $_bjermr1rjkmcwkij.matchNodeNames(
		...
		var isUnselectable = function(node) {
			return (
				$_bjermr1rjkmcwkij.isElement(node) &&
				node.getAttribute('unselectable') === 'true'
			);
		};
		var isNonUiContentEditableFalse = function(node) {
			return (
				isUnselectable(node) === false && isContentEditableFalse$1(node)
			);
		};
		...
		var isBlock$1 = $_bjermr1rjkmcwkij.matchStyleValues(
			'display',
			'block table'
		);
		var isFloated = $_bjermr1rjkmcwkij.matchStyleValues(
			'float',
			'left right'
		);
		var isValidElementCaretCandidate = $_fl2rax2ljkmcwkot.and(
			isElement$3,
			isCaretCandidate$1,
			$_fl2rax2ljkmcwkot.negate(isFloated)
		);
		var isNotPre = $_fl2rax2ljkmcwkot.negate(
			$_bjermr1rjkmcwkij.matchStyleValues(
				'white-space',
		...
		var getCaretPositionClientRects = function(caretPosition) {
			...
			var addUniqueAndValidRect = function(clientRect) {
				...
				if (clientRects.length > 0) {
					if (
						isEqual(clientRect, clientRects[clientRects.length - 1])
					) {
						return;
					}
				}
		...
		(function(CaretPosition) {
			...
			CaretPosition.isAbove = function(pos1, pos2) {
				return liftN(
					[head(pos2.getClientRects()), last(pos1.getClientRects())],
					isAbove
				).getOr(false);
			};
			CaretPosition.isBelow = function(pos1, pos2) {
				return liftN(
					[last(pos2.getClientRects()), head(pos1.getClientRects())],
					isBelow
				).getOr(false);
			};
		...
		var getChildNodes = function(node) {
			...
			return $_9g1fibmjkmcwkbu.reduce(
				node.childNodes,
		...
		var normalizedNodeIndex = function(node) {
			...
			numTextFragments = $_9g1fibmjkmcwkbu.reduce(
				nodes,
			...
			nodes = $_9g1fibmjkmcwkbu.filter(
				nodes,
				$_bjermr1rjkmcwkij.matchNodeNames(node.nodeName)
			);
			index = $_9g1fibmjkmcwkbu.findIndex(nodes, equal$1(node), node);
		...
		var create$2 = function(root, caretPosition) {
			...
			parents = $_9g1fibmjkmcwkbu.filter(
				parents,
				$_fl2rax2ljkmcwkot.negate($_bjermr1rjkmcwkij.isBogus)
			);
			path = path.concat(
				$_9g1fibmjkmcwkbu.map(parents, function(node) {
		...
		var resolvePathItem = function(node, name, index) {
			...
			nodes = $_9g1fibmjkmcwkbu.filter(
				nodes,
				$_bjermr1rjkmcwkij.matchNodeNames(name)
			);
			return nodes[index];
		};
		...
		var resolve$2 = function(root, path) {
			...
			container = $_9g1fibmjkmcwkbu.reduce(
				path,
		...
		var getNormalizedTextOffset = function(trim, container, offset) {
			...
			for (
				...
				node && $_bjermr1rjkmcwkij.isText(node);
				node = node.previousSibling
			) {
		...
		var moveEndPoint = function(rng, start) {
			...
			if (
				$_bjermr1rjkmcwkij.isElement(container) &&
		...
		var getBookmark = function(selection, type, normalized) {
			if (type === 2) {
				return getOffsetBookmark(
					$_cadvcr2pjkmcwkph.trim,
					normalized,
					selection
				);
			} else if (type === 3) {
		...
		var remove$7 = function(caretContainerNode) {
			...
			if (isText$6(caretContainerNode)) {
				var text = $_cadvcr2pjkmcwkph.trim(
					getNodeValue(caretContainerNode)
				);
		...
		var renderCaretAtRange = function(editor, range, scrollIntoView) {
			...
			var ceRoot = editor.dom.getParent(caretPosition.getNode(), function(
				node
			) {
				return (
		...
		var getCaretCandidatePosition = function(direction, node) {
			if (isForwards(direction)) {
				if (
					isCaretCandidate$2(node.previousSibling) &&
			...
			if (isBackwards(direction)) {
				if (
					isCaretCandidate$2(node.nextSibling) &&
		...
		var findCaretPosition = function(direction, startPos, root) {
			...
			if (
				...
				if (
					...
					isCaretCandidate$2(root.lastChild) &&
					isElement$5(root.lastChild)
				) {
			...
			if (
				...
				node = findNode(
					node,
					direction,
					$_fl2rax2ljkmcwkot.constant(true),
					root,
					true
				);
			...
			rootContentEditableFalseElm = $_9g1fibmjkmcwkbu.last(
				$_9g1fibmjkmcwkbu.filter(
		...
		var getAdjacentLinePositions = function(
			...
			return getPositionsUntilBreak(scope, start)
				.breakAt.map(function(pos) {
					var positions = getPositionsUntilBreak(scope, pos)
						.positions;
					return direction === HDirection.Backwards
		...
		var getClosestCell = function(
			...
			var cells = $_2gqm0n27jkmcwkmi
				.descendants(Element$$1.fromDom(table), 'td,th')
				.map(function(e) {
					return e.dom();
				});
			var corners = filter(getCorners(getYAxisValue, cells), function(
				corner
			) {
		...
		var getFontStyleValues = function(editor) {
			return $_4ujg4tljkmcwkbk.explode(
				editor.getParam('font_size_style_values', '')
			);
		};
		var getFontSizeClasses = function(editor) {
			return $_4ujg4tljkmcwkbk.explode(
				editor.getParam('font_size_classes', '')
			);
		};
		...
		var isAtFirstTableCellLine = $_fl2rax2ljkmcwkot.curry(
			isAtTableCellLine,
			getPositionsUntilPreviousLine
		);
		var isAtLastTableCellLine = $_fl2rax2ljkmcwkot.curry(
			isAtTableCellLine,
			getPositionsUntilNextLine
		);
		...
		var renderBlock = function(down, editor, table, pos) {
			...
			if (forcedRootBlock) {
				editor.undoManager.transact(function() {
					...
					$_29helg15jkmcwkf0.setAll(
						element,
						$_2ycm1438jkmcwkta.getForcedRootBlockAttrs(editor)
					);
					$_f4qt4v1yjkmcwkl2.append(
						element,
						Element$$1.fromTag('br')
					);
					if (down) {
						$_f4qt4v1yjkmcwkl2.after(
							Element$$1.fromDom(table),
							element
						);
					} else {
						$_f4qt4v1yjkmcwkl2.before(
							Element$$1.fromDom(table),
							element
						);
					}
		...
		var moveCaret = function(editor, down, pos) {
			...
			table.fold(
				...
				function(table) {
					return $_1m3f3j2yjkmcwkqg
						.positionIn(last$$1, editor.getBody())
		...
		var FakeCaret = function(root, isBlock, hasFocus) {
			...
			var show = function(before, element) {
				...
				if (isBlock(element)) {
					...
					rng = element.ownerDocument.createRange();
					if (
						isContentEditableFalse$5(caretContainerNode.nextSibling)
					) {
						rng.setStart(caretContainerNode, 0);
						rng.setEnd(caretContainerNode, 0);
					} else {
		...
		var isFakeCaretTarget = function(node) {
			return (
				isContentEditableFalse$5(node) ||
				($_bjermr1rjkmcwkij.isTable(node) && isFakeCaretTableBrowser())
			);
		};

		var isContentEditableFalse$6 =
			$_bjermr1rjkmcwkij.isContentEditableFalse;
		var isBlockLike = $_bjermr1rjkmcwkij.matchStyleValues(
			'display',
		...
		var isNextToContentEditableFalse = function(
			...
			return (
				isContentEditableFalse$6(node) &&
				!$_bjermr1rjkmcwkij.isBogusAll(node)
			);
		};
		var isNextToTable = function(relativeOffset, caretPosition) {
			return $_bjermr1rjkmcwkij.isTable(
		...
		var getRelativeCefElm = function(forward, caretPosition) {
			...
			).filter(isContentEditableFalse$6);
		};
		...
		var isBeforeContentEditableFalse = curry$2(
			isNextToContentEditableFalse,
			0
		);
		var isAfterContentEditableFalse = curry$2(
			isNextToContentEditableFalse,
			-1
		);
		...
		var afterElement = function(node) {
			return $_bjermr1rjkmcwkij.isBr(node)
				? CaretPosition$1.before(node)
		...
		var isAtBr = function(position) {
			return (
				...
				$_bjermr1rjkmcwkij.isBr(position.getNode())
			);
		};
		...
		var insertZwsp = function(node, rng) {
			var textNode = node.ownerDocument.createTextNode(
				$_cadvcr2pjkmcwkph.ZWSP
			);
		...
		var tryFindRangePosition = function(node, rng) {
			return $_1m3f3j2yjkmcwkqg.lastPositionIn(node).fold(
				function() {
					return false;
				},
				function(pos) {
		...
		var restoreEndPoint = function(dom, suffix, bookmark) {
			...
			if (marker) {
				...
				if (!keep) {
					...
					$_4ujg4tljkmcwkbk.each(
						$_4ujg4tljkmcwkbk.grep(marker.childNodes),
						function(node) {
					...
					if (
						...
						$_bjermr1rjkmcwkij.isText(prev) &&
						!$_cvvl9iajkmcwk25.opera
					) {
		...
		var resolveId = function(dom, bookmark) {
			...
			return liftN([startPos, alt(endPos, startPos)], function(
				spos,
				epos
			) {
				var rng = dom.createRng();
		...
		var resolveIndex$1 = function(dom, bookmark) {
			return Option.from(dom.select(bookmark.name)[bookmark.index]).map(
				function(elm) {
					var rng = dom.createRng();
					rng.selectNode(elm);
					return rng;
		...
		var isZeroWidth = function(elem) {
			return (
				$_2xwdjv11jkmcwke0.isText(elem) &&
				$_ckzc193ejkmcwktu.get(elem) === zeroWidth()
			);
		};
		var context = function(editor, elem, wrapName, nodeName) {
			return $_ca6pc118jkmcwkfs.parent(elem).fold(
				function() {
					return 'skipping';
				},
				function(parent) {
		...
		var applyWordGrab = function(editor, rng) {
			var r = $_97afwh2ujkmcwkpw.expandRng(
				editor,
				rng,
				[{ inline: true }],
				false
		...
		var annotateWithBookmark = function(editor, name, settings, data) {
			editor.undoManager.transact(function() {
				...
				var bookmark = $_aw3zi52jjkmcwkoa.getPersistentBookmark(
					editor.selection,
					true
				);
				var rng = editor.selection.getRng();
		...
		var ElementUtils = function(dom) {
			this.compare = function(node1, node2) {
				...
				return (
					!$_e4btcc2vjkmcwkq3.isBookmarkNode(node1) &&
					!$_e4btcc2vjkmcwkq3.isBookmarkNode(node2)
				);
			};
		};
		...
		var fillWithPaddingBr = function(elm) {
			...
			$_f4qt4v1yjkmcwkl2.append(
				elm,
				Element$$1.fromHtml('<br data-mce-bogus="1">')
			);
		};
		var isPaddingContents = function(elm) {
			return $_2xwdjv11jkmcwke0.isText(elm)
				? $_ckzc193ejkmcwktu.get(elm) === '\xA0'
				: isBr(elm);
		};
		var isPaddedElement = function(elm) {
			return (
				filter($_ca6pc118jkmcwkfs.children(elm), isPaddingContents)
					.length === 1
			);
		};
		var trimBlockTrailingBr = function(elm) {
			$_ca6pc118jkmcwkfs.lastChild(elm).each(function(lastChild) {
				$_ca6pc118jkmcwkfs
					...
					.each(function(lastChildPrevSibling) {
						if (
							isBlock(elm) &&
							isBr(lastChild) &&
		...
		function Writer(settings) {
			...
			encode = $_33qwr1sjkmcwkip.getEncodeFunc(
		...
		var normalizeBlockSelectionRange = function(rng) {
			...
			return $_1m3f3j2yjkmcwkqg
				.fromPosition(false, rootNode, endPos)
		...
		var trimBrsFromTableCell = function(dom, elm) {
			Option.from(dom.getParent(elm, 'td,th'))
				...
				.each($_zygeg3ljkmcwkvo.trimBlockTrailingBr);
		};
		var insertHtmlAtCaret = function(editor, value, details) {
			...
			var umarkFragmentElements = function(elm) {
				$_4ujg4tljkmcwkbk.each(elm.getElementsByTagName('*'), function(
					elm
				) {
			...
			var canHaveChildren = function(node) {
				return (
					node &&
					!editor.schema.getShortEndedElements()[node.nodeName]
				);
			};
			...
			if (caretElement === body && selection.isCollapsed()) {
				if (
					...
					canHaveChildren(body.firstChild) &&
			...
			if (!selection.isCollapsed()) {
				editor.selection.setRng(
					$_bx30le3ojkmcwkw9.normalize(editor.selection.getRng())
				);
			...
			if (
				details.paste === true &&
				$_e6q31u3jjkmcwkvd.isListFragment(editor.schema, fragment) &&
				$_e6q31u3jjkmcwkvd.isParentBlockLi(dom, parentNode)
			) {
				rng = $_e6q31u3jjkmcwkvd.insertAtCaret(
					serializer,
					dom,
		...
		var getExternalPlugins = function(overrideSettings, settings) {
			...
			if (overrideSettings && overrideSettings.external_plugins) {
				return $_4ujg4tljkmcwkbk.extend(
					{},
		...
		var processPlugins = function(
			...
			return $_4ujg4tljkmcwkbk.extend(settings, {
		...
		var combineSettings = function(
			...
			var extendedSettings = $_4ujg4tljkmcwkbk.extend(
				defaultSettings,
				defaultOverrideSettings,
				sectionResult.settings(),
				isOnMobile(isTouchDevice, sectionResult)
					? getSection(sectionResult, 'mobile', defaultMobileSettings)
					: {},
				{
					validate: true,
			...
			if (typeof value === 'string') {
				each(
					...
					function(val) {
						...
						if (arr.length > 1) {
							output[
								$_4ujg4tljkmcwkbk.trim(arr[0])
							] = $_4ujg4tljkmcwkbk.trim(arr[1]);
						} else {
							output[
								$_4ujg4tljkmcwkbk.trim(arr[0])
							] = $_4ujg4tljkmcwkbk.trim(arr);
						}
					}
				);
			} else {
		...
		var isRtl = function(element) {
			return (
				DOMUtils$1.DOM.getStyle(element, 'direction', true) === 'rtl' ||
		...
		var isBeforeRoot = function(rootNode) {
			return function(elm) {
				return $_e4hu6h1ejkmcwkgj.eq(
					rootNode,
					Element$$1.fromDom(elm.dom().parentNode)
				);
			};
		};
		var getParentBlock$1 = function(rootNode, elm) {
			return $_e4hu6h1ejkmcwkgj.contains(rootNode, elm)
				? $_253nd92ajkmcwkmp.closest(
						elm,
						function(element) {
		...
		var willDeleteLastPositionInElement = function(forward, fromPos, elm) {
			return liftN(
				...
				function(firstPos, lastPos) {
					var normalizedFirstPos = $_crqmqb3tjkmcwkxd.normalizePosition(
						true,
						firstPos
					);
					var normalizedLastPos = $_crqmqb3tjkmcwkxd.normalizePosition(
						false,
						lastPos
					);
					var normalizedFromPos = $_crqmqb3tjkmcwkxd.normalizePosition(
						false,
						fromPos
					);
					if (forward) {
						return $_1m3f3j2yjkmcwkqg
							.nextPosition(elm, normalizedFromPos)
						...
						return $_1m3f3j2yjkmcwkqg
							.prevPosition(elm, normalizedFromPos)
		...
		var hasWhitespacePreserveParent = function(rootNode, node) {
			...
			return $_6rijl23xjkmcwl24.ancestor(
				startNode,
				'pre,code',
				curry($_e4hu6h1ejkmcwkgj.eq, rootElement)
			);
		};
		...
		var isBogusAll$1 = $_bjermr1rjkmcwkij.hasAttributeValue(
			'data-mce-bogus',
			'all'
		);
		...
		var getBlockPosition = function(rootNode, pos) {
			...
			return $_8z75uj3sjkmcwkx3
				.getParentBlock(rootElm, containerElm)
				.map(function(block) {
		...
		var hasSameParent = function(blockBoundary) {
			return $_ca6pc118jkmcwkfs
				.parent(blockBoundary.from().block())
				.bind(function(parent1) {
					return $_ca6pc118jkmcwkfs
						.parent(blockBoundary.to().block())
						.filter(function(parent2) {
		...
		var skipLastBr = function(rootNode, forward, blockPosition) {
			if (
				$_bjermr1rjkmcwkij.isBr(blockPosition.position().getNode()) &&
				$_1jnfso3wjkmcwl1v.isEmpty(blockPosition.block()) === false
			) {
				return $_1m3f3j2yjkmcwkqg
					.positionIn(false, blockPosition.block().dom())
		...
		var readFromRange = function(rootNode, forward, rng) {
			...
			var toBlockPos = fromBlockPos.bind(function(blockPos) {
				return $_1m3f3j2yjkmcwkqg
					.fromPosition(forward, rootNode, blockPos.position())
					.bind(function(to) {
			...
			return liftN([fromBlockPos, toBlockPos], BlockBoundary).filter(
		...
		var parentsUntil$1 = function(startNode, rootElm, predicate) {
			if ($_e4hu6h1ejkmcwkgj.contains(rootElm, startNode)) {
				return dropLast(
					$_ca6pc118jkmcwkfs.parents(startNode, function(elm) {
						return (
							predicate(elm) ||
							$_e4hu6h1ejkmcwkgj.eq(elm, rootElm)
						);
					})
				);
			} else {
		...
		var getChildrenUntilBlockBoundary = function(block) {
			...
			return findIndex(children, isBlock).fold(
				function() {
					return children;
				},
				function(index) {
		...
		var removeEmptyRoot = function(rootNode, block) {
			...
			return find(parents.reverse(), $_1jnfso3wjkmcwl1v.isEmpty).each(
				$_eualm21wjkmcwkku.remove
			);
		};
			...
			if ($_e4hu6h1ejkmcwkgj.contains(toBlock, fromBlock)) {
				return $_ca6pc118jkmcwkfs
					...
					.bind(function(parent) {
						return $_e4hu6h1ejkmcwkgj.eq(parent, toBlock)
							? Option.some(fromBlock)
							: findParentInsertPoint(toBlock, fromBlock);
					});
			} else {
				return Option.none();
			}
			...
			if ($_1jnfso3wjkmcwl1v.isEmpty(toBlock)) {
				...
				return getInsertionPoint(fromBlock, toBlock).fold(
					function() {
						...
						var position = $_1m3f3j2yjkmcwkqg.lastPositionIn(
							toBlock.dom()
						);
					...
					function(target) {
						var position = $_1m3f3j2yjkmcwkqg.prevPosition(
							toBlock.dom(),
							CaretPosition$1.before(target.dom())
						);
						each(children_1, function(node) {
		...
		var backspaceDelete = function(editor, forward) {
			...
			position = $_2x2kic3rjkmcwkwi
				.read(rootNode.dom(), forward, editor.selection.getRng())
				.bind(function(blockBoundary) {
					return $_ezyxdl3yjkmcwl27.mergeBlocks(
						rootNode,
						forward,
						blockBoundary.from().block(),
						blockBoundary.to().block()
		...
		var deleteRangeMergeBlocks = function(rootNode, selection) {
			...
			return liftN(
				[
					$_8z75uj3sjkmcwkx3.getParentBlock(
						rootNode,
				...
				function(block1, block2) {
					if ($_e4hu6h1ejkmcwkgj.eq(block1, block2) === false) {
						rng.deleteContents();
						$_ezyxdl3yjkmcwl27
							.mergeBlocks(rootNode, true, block1, block2)
							.each(function(pos) {
		...
		var isRawNodeInTable = function(root, rawNode) {
			...
			return $_253nd92ajkmcwkmp
				.ancestor(node, isTableCell, isRoot)
				.isSome();
		};
		...
		var isEverythingSelected = function(root, rng) {
			var noPrevious = $_1m3f3j2yjkmcwkqg
				.prevPosition(root.dom(), CaretPosition$1.fromRangeStart(rng))
				.isNone();
			var noNext = $_1m3f3j2yjkmcwkqg
				.nextPosition(root.dom(), CaretPosition$1.fromRangeEnd(rng))
				.isNone();
		...
		var isCompoundElement = function(node) {
			return (
				isTableCell(Element$$1.fromDom(node)) ||
				isListItem(Element$$1.fromDom(node))
			);
		};
		...
		var isAtContentEditableBlockCaret = function(forward, from) {
			...
			return (
				$_bjermr1rjkmcwkij.isElement(elm) &&
		...
		var isDeleteFromCefDifferentBlocks = function(root, forward, from, to) {
			var inSameBlock = function(elm) {
				return (
					isInline(Element$$1.fromDom(elm)) &&
					!isInSameBlock(from, to, root)
				);
			};
		...
		var deleteEmptyBlockOrMoveToCef = function(root, forward, from, to) {
			...
			return $_8z75uj3sjkmcwkx3
				.getParentBlock(
					...
					Element$$1.fromDom(from.getNode())
				)
				.map(function(blockElm) {
					return $_1jnfso3wjkmcwl1v.isEmpty(blockElm)
		...
		var findCefPosition = function(root, forward, from) {
			return $_1m3f3j2yjkmcwkqg
				.fromPosition(forward, root, from)
				.bind(function(to) {
					if (isCompoundElement(to.getNode())) {
						return Option.none();
					} else if (
						isDeleteFromCefDifferentBlocks(root, forward, from, to)
					) {
						return Option.none();
					} else if (
						forward &&
						$_bjermr1rjkmcwkij.isContentEditableFalse(to.getNode())
					) {
		...
		var getContentEditableBlockAction = function(forward, elm) {
			if (
				forward &&
				$_bjermr1rjkmcwkij.isContentEditableFalse(elm.nextSibling)
			) {
				return Option.some(DeleteAction.moveToElement(elm.nextSibling));
			} else if (
				forward === false &&
				$_bjermr1rjkmcwkij.isContentEditableFalse(elm.previousSibling)
			) {
				return Option.some(
		...
		var getPreviousSiblingCaretPosition = function(elm) {
			if (isCaretCandidate(elm.previousSibling)) {
				...
				return elm.previousSibling
					? $_1m3f3j2yjkmcwkqg.lastPositionIn(elm.previousSibling)
					: Option.none();
			}
		};
		var getNextSiblingCaretPosition = function(elm) {
			if (isCaretCandidate(elm.nextSibling)) {
				...
				return elm.nextSibling
					? $_1m3f3j2yjkmcwkqg.firstPositionIn(elm.nextSibling)
					: Option.none();
			}
		};
		var findCaretPositionBackwardsFromElm = function(rootElement, elm) {
			...
			return $_1m3f3j2yjkmcwkqg
				.prevPosition(rootElement, startPosition)
				.fold(function() {
					return $_1m3f3j2yjkmcwkqg.nextPosition(
						rootElement,
		...
		var findCaretPositionForwardsFromElm = function(rootElement, elm) {
			return $_1m3f3j2yjkmcwkqg
				.nextPosition(rootElement, CaretPosition$1.after(elm))
				.fold(function() {
					return $_1m3f3j2yjkmcwkqg.prevPosition(
						rootElement,
		...
		var isBlock$2 = function(editor, elm) {
			return (
				...
				editor.schema
					.getBlockElements()
					.hasOwnProperty($_2xwdjv11jkmcwke0.name(elm))
			);
		};
		...
		var deleteNormalized = function(elm, afterDeletePosOpt) {
			return liftN(
				...
				function(prev, next, afterDeletePos) {
					...
					if (
						$_bjermr1rjkmcwkij.isText(prevNode) &&
						$_bjermr1rjkmcwkij.isText(nextNode)
					) {
		...
		var deleteElement = function(editor, forward, elm) {
			...
			var parentBlock = $_253nd92ajkmcwkmp.ancestor(
				elm,
		...
		var deleteElement$1 = function(editor, forward) {
			return function(element) {
				...
				$_4yq1i444jkmcwl3w.deleteElement(
					editor,
					forward,
					Element$$1.fromDom(element)
				);
				return true;
			};
		};
		...
		var backspaceDeleteRange = function(editor, forward) {
			...
			if ($_bjermr1rjkmcwkij.isContentEditableFalse(selectedElement)) {
				...
				$_4yq1i444jkmcwl3w.deleteElement(
					editor,
					forward,
					Element$$1.fromDom(editor.selection.getNode())
				);
		...
		var getContentEditableRoot = function(root, node) {
			while (node && node !== root) {
				if (
					$_bjermr1rjkmcwkij.isContentEditableTrue(node) ||
					$_bjermr1rjkmcwkij.isContentEditableFalse(node)
				) {
					return node;
				}
				node = node.parentNode;
		...
		var paddEmptyElement = function(editor) {
			...
			if (
				$_bjermr1rjkmcwkij.isContentEditableTrue(ceRoot) &&
		...
		var before$3 = function(isInlineTarget, rootNode, pos) {
			...
			return $_crqmqb3tjkmcwkxd
				.findRootInline(isInlineTarget, scope, nPos)
				.fold(function() {
					return $_1m3f3j2yjkmcwkqg
						.nextPosition(scope, nPos)
						.bind(
							curry(
								$_crqmqb3tjkmcwkxd.findRootInline,
								isInlineTarget,
								scope
							)
						)
		...
		var findInsideRootInline = function(isInlineTarget, rootNode, pos) {
			return $_crqmqb3tjkmcwkxd
				.findRootInline(isInlineTarget, rootNode, pos)
		...
		var betweenInlines = function(
			...
			return liftN(
				...
				function(fromInline, toInline) {
					if (
						fromInline !== toInline &&
						$_crqmqb3tjkmcwkxd.hasSameParentBlock(
							rootNode,
							fromInline,
							toInline
						)
		...
		var findLocationTraverse = function(
			...
			var to = $_1m3f3j2yjkmcwkqg
				.fromPosition(forward, rootNode, from)
				.map(curry($_crqmqb3tjkmcwkxd.normalizePosition, forward));
		...
		var moveRel = function(forward, selection, pos) {
			...
			selection.setRng(
				CaretPosition$1(pos.container(), pos.offset() + delta).toRange()
			);
			selection
				.getSel()
		...
		var renderCaretLocation = function(editor, caret, location) {
			return $_efgx6546jkmcwl4n
				.renderCaret(caret, location)
				.map(function(pos) {
		...
		var findLocation$1 = function(editor, caret, forward) {
			...
			var isInlineTarget = curry(
				$_crqmqb3tjkmcwkxd.isInlineTarget,
				editor
			);
			var location = $_gau9ep48jkmcwl4x.findLocation(
				forward,
				isInlineTarget,
				rootNode,
				from
		...
		var safeRemoveCaretContainer = function(editor, caret) {
			if (
				...
				if (
					...
					$_crqmqb3tjkmcwkxd.isAtZwsp(pos) === false
				) {
					setCaretPosition(
						editor,
						$_xl2r331jkmcwkri.removeAndReposition(caret.get(), pos)
					);
					caret.set(null);
				}
			}
		...
		var renderInsideInlineCaret = function(
			...
			if (editor.selection.isCollapsed()) {
				...
				each(inlines, function(inline) {
					...
					$_gau9ep48jkmcwl4x
						.readLocation(isInlineTarget, editor.getBody(), pos)
		...
		var moveWord = function(forward, editor, caret) {
			return function() {
				return isFeatureEnabled(editor)
					? $_dessn94bjkmcwl5p.moveByWord(forward, editor)
					: false;
			};
		};
		...
		var hasOnlyTwoOrLessPositionsLeft = function(elm) {
			return liftN(
				...
				function(firstPos, lastPos) {
					...
					return $_1m3f3j2yjkmcwkqg
						.nextPosition(elm, normalizedFirstPos)
						.map(function(pos) {
		...
		var backspaceDeleteCollapsed = function(editor, caret, forward, from) {
			...
			var fromLocation = $_gau9ep48jkmcwl4x.readLocation(
				isInlineTarget,
				rootNode,
				from
			);
			return fromLocation
				.bind(function(location$$1) {
					if (forward) {
						return location$$1.fold(
							constant(
								Option.some(
									$_gau9ep48jkmcwl4x.inside(location$$1)
								)
							),
							Option.none,
							constant(
								Option.some(
									$_gau9ep48jkmcwl4x.outside(location$$1)
								)
							),
							Option.none
						);
				...
				.map(setCaretLocation(editor, caret))
				.getOrThunk(function() {
					var toPosition = $_1m3f3j2yjkmcwkqg.navigate(
						forward,
						rootNode,
						from
					);
					var toLocation = toPosition.bind(function(pos) {
						return $_gau9ep48jkmcwl4x.readLocation(
							isInlineTarget,
							rootNode,
							pos
						);
		...
		var getClosestCell$1 = function(container, isRoot) {
			return $_4ki4d929jkmcwkmn.closest(
				Element$$1.fromDom(container),
				'td,th',
				isRoot
			);
		};
		...
		var isExpandedCellRng = function(cellRng) {
			return (
				$_e4hu6h1ejkmcwkgj.eq(cellRng.start(), cellRng.end()) === false
			);
		};
		...
		var partialSelection = function(isRoot, rng) {
			...
			return rng.collapsed
				? Option.none()
				: liftN([startCell, endCell], tableCellRng).fold(
						function() {
							return startCell.fold(
								...
								function(startCell) {
									return getClosestTable(
										...
										return last(getTableCells(table)).map(
											function(endCell) {
												return tableCellRng(
													startCell,
													endCell
		...
		var getCellRng = function(rng, isRoot) {
			var startCell = getClosestCell$1(rng.startContainer, isRoot);
			var endCell = getClosestCell$1(rng.endContainer, isRoot);
			return liftN([startCell, endCell], tableCellRng)
		...
		var getCellsFromRanges = function(ranges) {
			return filter(
				$_admly54fjkmcwl6u.getSelectedNodes(ranges),
				isTableCell
			);
		};
		var getCellsFromElement = function(elm) {
			var selectedCells = $_2gqm0n27jkmcwkmi.descendants(
				elm,
		...
		var getCellsFromEditor = function(editor) {
			return getCellsFromElementOrRanges(
				$_admly54fjkmcwl6u.getRanges(editor.selection.getSel()),
		...
		var deleteCellRange = function(editor, rootElm, rng) {
			return $_2x2sml4djkmcwl68
				.getActionFromRange(rootElm, rng)
				.map(function(action) {
					return action.fold(
		...
		var getParentCell = function(rootElm, elm) {
			return find(
				$_cir6zl3zjkmcwl2j.parentsAndSelf(elm, rootElm),
				isTableCell
			);
		};
			...
			return $_1m3f3j2yjkmcwkqg
				...
				.bind(function(to) {
					return getParentCell(
						rootElm,
						Element$$1.fromDom(to.getNode())
					).map(function(toCell) {
						return (
							$_e4hu6h1ejkmcwkgj.eq(toCell, fromCell) === false
						);
					});
				});
		};
		...
		var isDeleteOfLastCharPos = function(fromCaption, forward, from, to) {
			return $_1m3f3j2yjkmcwkqg
				...
				.bind(function(first) {
					return $_1m3f3j2yjkmcwkqg
						.lastPositionIn(fromCaption.dom())
						.map(function(last$$1) {
							return forward
								? from.isEqual(first) && to.isEqual(last$$1)
								: from.isEqual(last$$1) && to.isEqual(first);
						});
				})
				.getOr(true);
		};
		...
		var deleteCaretCells = function(editor, forward, rootElm, startElm) {
			...
			return getParentCell(rootElm, startElm).bind(function(fromCell) {
				return $_1jnfso3wjkmcwl1v.isEmpty(fromCell)
		...
		var deleteCaretCaption = function(
			...
			return $_1jnfso3wjkmcwl1v.isEmpty(fromCaption)
		...
		var getFontProp = function(propName) {
			return function(rootElm, elm) {
				return Option.from(elm)
					.map(Element$$1.fromDom)
					.filter($_2xwdjv11jkmcwke0.isElement)
		...
		var findFirstCaretElement = function(editor) {
			return $_1m3f3j2yjkmcwkqg
				.firstPositionIn(editor.getBody())
				.map(function(caret) {
					var container = caret.container();
					return $_bjermr1rjkmcwkij.isText(container)
		...
		var fromFontSizeNumber = function(editor, value) {
			if (/^[0-9\.]+$/.test(value)) {
				...
				if (fontSizeNumber >= 1 && fontSizeNumber <= 7) {
					var fontSizes = $_2ycm1438jkmcwkta.getFontStyleValues(
						editor
					);
					var fontClasses = $_2ycm1438jkmcwkta.getFontSizeClasses(
						editor
					);
					if (fontClasses) {
				...
				function() {
					return findFirstCaretElement(editor)
						.map(function(caretElement) {
							return $_3qs0f64hjkmcwl74.getFontFamily(
								editor.getBody(),
								caretElement
							);
						})
		...
		var fontSizeQuery = function(editor) {
			return getCaretElement(editor).fold(
				function() {
					return findFirstCaretElement(editor)
						.map(function(caretElement) {
							return $_3qs0f64hjkmcwl74.getFontSize(
								editor.getBody(),
								caretElement
							);
						})
		...
		var findTextNodeRelative = function(
			...
			if (
				left &&
				$_bjermr1rjkmcwkij.isBr(startNode) &&
				isAfterNode &&
		...
		var normalizeEndPoint = function(dom, collapsed, start, rng) {
			...
			if (container === body) {
				...
				if (container.hasChildNodes()) {
					...
					container = container.childNodes[offset];
					offset =
						$_bjermr1rjkmcwkij.isText(container) && isAfterNode
		...
		var normalize$1 = function(dom, rng) {
			...
			if (!collapsed) {
				normalizeEndPoint(dom, collapsed, false, normRng).each(function(
					pos
				) {
			...
			return $_axtxv74kjkmcwl84.isEq(rng, normRng)
				? Option.none()
				: Option.some(normRng);
		};
		...
		var insertBrAtCaret = function(editor, evt) {
			...
			if (
				...
				if (
					!hasRightSideContent(editor.schema, container, parentBlock)
				) {
					brElm = dom.create('br');
		...
		var hasBrAfter = function(rootNode, startNode) {
			if (isBeforeBr(CaretPosition$1.after(startNode))) {
				...
				return $_1m3f3j2yjkmcwkqg
					.nextPosition(rootNode, CaretPosition$1.after(startNode))
					.map(function(pos) {
						return $_bjermr1rjkmcwkij.isBr(pos.getNode());
					})
					.getOr(false);
		...
		var readInlineAnchorLocation = function(editor) {
			...
			return $_gau9ep48jkmcwl4x
				.readLocation(isInlineTarget, editor.getBody(), position)
		...
		var clamp = function(offset, element) {
			var max = $_2xwdjv11jkmcwke0.isText(element)
				? $_ckzc193ejkmcwktu.get(element).length
				: $_ca6pc118jkmcwkfs.children(element).length + 1;
			if (offset > max) {
				return max;
			} else if (offset < 0) {
				return 0;
		...
		var normalizeRng = function(rng) {
			return $_2v8ne64mjkmcwl8l.range(
				rng.start(),
		...
		var isOrContains = function(root, elm) {
			return (
				$_e4hu6h1ejkmcwkgj.contains(root, elm) ||
				$_e4hu6h1ejkmcwkgj.eq(root, elm)
			);
		};
		...
		var nativeRangeToSelectionRange = function(r) {
			return $_2v8ne64mjkmcwl8l.range(
				Element$$1.fromDom(r.startContainer),
				r.startOffset,
				Element$$1.fromDom(r.endContainer),
				r.endOffset
			);
		};
		...
		var store = function(editor) {
			var newBookmark = shouldStore(editor)
				? getBookmark$2(Element$$1.fromDom(editor.getBody()))
				: Option.none();
		...
		var getRng = function(editor) {
			...
			return bookmark
				.bind(curry(validate, Element$$1.fromDom(editor.getBody())))
		...
		function EditorCommands(editor) {
			...
			var addCommands = function(commandList, type) {
				...
				each$10(commandList, function(callback, command) {
					each$10(command.toLowerCase().split(','), function(
						command
					) {
			...
			addCommands({
				...
				'JustifyLeft,JustifyCenter,JustifyRight,JustifyFull,JustifyNone': function(
					...
					each$10('left,center,right,justify'.split(','), function(
						name
					) {
						if (align !== name) {
				...
				'InsertUnorderedList,InsertOrderedList': function(command) {
					...
					if (listElm) {
						...
						if (
							/^(H[1-6]|P|ADDRESS|PRE)$/.test(listParent.nodeName)
						) {
							storeSelection();
				...
				selectAll: function() {
					var editingHost = dom.getParent(
						selection.getStart(),
						$_bjermr1rjkmcwkij.isContentEditableTrue
					);
					if (editingHost) {
						var rng = dom.createRng();
		...
		var nativeEvents = $_4ujg4tljkmcwkbk.makeMap(
		...
		var EditorObservable = {
			bindPendingEventDelegates: function() {
				var self = this;
				$_4ujg4tljkmcwkbk.each(self._pendingNativeEvents, function(
					name
				) {
		...
		EditorObservable = $_4ujg4tljkmcwkbk.extend(
			{},
			$_ed2ug44qjkmcwl97,
			EditorObservable
		);
		...
		var modifierNames = $_4ujg4tljkmcwkbk.makeMap(
		...
		function Shortcuts(editor) {
			...
			var createShortcut = function(pattern, desc, cmdFunc, scope) {
				var shortcuts;
				shortcuts = $_4ujg4tljkmcwkbk.map(
					explode$2(pattern, '>'),
					parseShortcut
				);
				shortcuts[shortcuts.length - 1] = $_4ujg4tljkmcwkbk.extend(
			...
			self.add = function(pattern, desc, cmdFunc, scope) {
				...
				each$11(
					explode$2($_4ujg4tljkmcwkbk.trim(pattern.toLowerCase())),
					function(pattern) {
		...
		var focusInside = function(element) {
			...
			var inside = active(doc).filter(function(a) {
				return $_szg1f4xjkmcwlad.closest(
					a,
					curry($_e4hu6h1ejkmcwkgj.eq, element)
				);
			});
		...
		var search = function(element) {
			return active($_ca6pc118jkmcwkfs.owner(element)).filter(function(
				e
			) {
		...
		var getCollapsedNode = function(rng) {
			return rng.collapsed
				? Option.from(getNode(rng.startContainer, rng.startOffset)).map(
						Element$$1.fromDom
				  )
				: Option.none();
		};
		...
		var normalizeSelection = function(editor, rng) {
			getFocusInElement(Element$$1.fromDom(editor.getBody()), rng)
				.bind(function(elm) {
		...
		var hasElementFocus = function(elm) {
			return (
				$_468a0m4wjkmcwla6.hasFocus(elm) ||
				$_468a0m4wjkmcwla6.search(elm).isSome()
			);
		};
		var hasIframeFocus = function(editor) {
			return (
				editor.iframeElement &&
				$_468a0m4wjkmcwla6.hasFocus(
					Element$$1.fromDom(editor.iframeElement)
				)
			);
		};
		...
		var isXYInContentArea = function(editor, clientX, clientY) {
			...
			var targetElm = editor.inline
				? bodyElm
				: $_ca6pc118jkmcwkfs.documentElement(bodyElm);
		...
		var isEditorAttachedToDom = function(editor) {
			...
			return fromDomSafe(rawContainer)
				.map(function(container) {
					return $_e4hu6h1ejkmcwkgj.contains(
						$_ca6pc118jkmcwkfs.owner(container),
						container
					);
				})
				.getOr(false);
		...
		function NotificationManager(editor) {
			...
			var open = function(args) {
				if (
					editor.removed ||
					!$_8we2e850jkmcwlaw.isEditorAttachedToDom(editor)
				) {
					return;
				}
		...
		var resolvePluginName = function(targetUrl, suffix) {
			for (var name$$1 in PluginManager.urls) {
				var matchUrl =
					PluginManager.urls[name$$1] + '/plugin' + suffix + '.js';
		...
		function Uploader(uploadStatus, settings) {
			...
			var resolvePending = function(blobUri, result) {
				$_4ujg4tljkmcwkbk.each(pendingPromises[blobUri], function(
					resolve
				) {
					resolve(result);
				});
			...
			var uploadBlobs = function(blobInfos, openNotification) {
				blobInfos = $_4ujg4tljkmcwkbk.grep(blobInfos, function(
					blobInfo
				) {
					return !uploadStatus.isUploaded(blobInfo.blobUri());
				});
				return promiseObj.all(
					$_4ujg4tljkmcwkbk.map(blobInfos, function(blobInfo) {
		...
		var imageToBlobInfo = function(blobCache, img, resolve, reject) {
			...
			if (img.src.indexOf('blob:') === 0) {
				...
				if (blobInfo) {
					...
					$_7eufcj5djkmcwlcv.uriToBlob(img.src).then(
						function(blob) {
							$_7eufcj5djkmcwlcv
								.blobToDataUri(blob)
								.then(function(dataUri) {
									base64 = $_7eufcj5djkmcwlcv.parseDataUri(
										dataUri
									).data;
		...
		function ImageScanner(uploadStatus, blobCache) {
			...
			var findAll = function(elm, predicate) {
				...
				images = $_9g1fibmjkmcwkbu.filter(getAllImages(elm), function(
					img
				) {
					var src = img.src;
		...
		function BlobCache() {
			...
			var toBlobInfo = function(o) {
				...
				return {
					...
					blobUri: constant(
						o.blobUri || $_ci16mbjkmcwk2b.createObjectURL(o.blob)
					),
					uri: constant(o.uri)
				};
			};
		...
		function EditorUpload(editor) {
			...
			var replaceUrlInUndoStack = function(targetUrl, replacementUrl) {
				$_9g1fibmjkmcwkbu.each(editor.undoManager.data, function(
					level
				) {
					if (level.type === 'fragmented') {
						level.fragments = $_9g1fibmjkmcwkbu.map(
							level.fragments,
							function(fragment) {
			...
			var replaceImageUri = function(image, resultUri) {
				...
				editor.$(image).attr({
					src: settings.images_reuse_filename
			...
			var uploadImages = function(callback) {
				...
				return scanForImages().then(
					aliveGuard(function(imageInfos) {
						var blobInfos;
						blobInfos = $_9g1fibmjkmcwkbu.map(imageInfos, function(
							imageInfo
						) {
						...
						return uploader
							...
							.then(
								aliveGuard(function(result) {
									var filteredResult = $_9g1fibmjkmcwkbu.map(
										result,
			...
			var scanForImages = function() {
				...
				return imageScanner
					...
					.then(
						aliveGuard(function(result) {
							result = $_9g1fibmjkmcwkbu.filter(result, function(
								resultItem
							) {
								if (typeof resultItem === 'string') {
									$_b9je0i54jkmcwlbe.displayError(
										editor,
										resultItem
									);
									return false;
							...
							$_9g1fibmjkmcwkbu.each(result, function(
								resultItem
							) {
			...
			var replaceBlobUris = function(content) {
				return content.replace(/src="(blob:[^"]+)"/g, function(
					match,
					blobUri
				) {
					...
					if (!blobInfo) {
						blobInfo = $_9g1fibmjkmcwkbu.reduce(
		...
		var isValidTarget = function(blockElements, node) {
			if ($_bjermr1rjkmcwkij.isText(node)) {
				...
				return (
					...
					!$_e4btcc2vjkmcwkq3.isBookmarkNode(node)
				);
			} else {
				return false;
			}
		...
		var hasBlockParent = function(blockElements, root, node) {
			return exists(
				$_cir6zl3zjkmcwl2j.parents(
					Element$$1.fromDom(node),
					Element$$1.fromDom(root)
				),
				function(elm) {
		...
		var getStartNode = function(rng) {
			...
			if ($_bjermr1rjkmcwkij.isText(sc)) {
				return so === 0
					? Option.some(Element$$1.fromDom(sc))
					: Option.none();
			} else {
		...
		var getEndNode = function(rng) {
			...
			if ($_bjermr1rjkmcwkij.isText(ec)) {
				return eo === ec.data.length
					? Option.some(Element$$1.fromDom(ec))
					: Option.none();
			} else {
		...
		var getFirstChildren = function(node) {
			return $_ca6pc118jkmcwkfs
				.firstChild(node)
				.fold(constant([node]), function(child) {
		...
		var hasAllContentsSelected = function(elm, rng) {
			return liftN([getStartNode(rng), getEndNode(rng)], function(
				startNode,
				endNode
			) {
				var start = find(
					getFirstChildren(elm),
					curry($_e4hu6h1ejkmcwkgj.eq, startNode)
				);
				var end = find(
					getLastChildren$1(elm),
					curry($_e4hu6h1ejkmcwkgj.eq, endNode)
				);
		...
		var moveEndPoint$1 = function(dom, rng, node, start) {
			...
			do {
				if (
					node.nodeType === 3 &&
					$_4ujg4tljkmcwkbk.trim(node.nodeValue).length !== 0
				) {
					if (start) {
						rng.setStart(node, 0);
					} else {
		...
		function NodeChange(editor) {
			...
			if (!('onselectionchange' in editor.getDoc())) {
				editor.on('NodeChange Click MouseUp KeyUp Focus', function(e) {
					...
					if (
						e.type === 'nodechange' ||
						!$_axtxv74kjkmcwl84.isEq(fakeRng, lastRng)
					) {
			...
			editor.on('SelectionChange', function() {
				...
				if (
					!startElm ||
					(!$_cvvl9iajkmcwk25.range && editor.selection.isCollapsed())
				) {
					return;
				}
				if (
		...
		var start$1 = function(state, editor) {
			return function(e) {
				if (isLeftMouseButtonPressed(e)) {
					var ceElm = $_9g1fibmjkmcwkbu.find(
						editor.dom.getParents(e.target),
						$_fl2rax2ljkmcwkot.or(
		...
		var move$1 = function(state, editor) {
			var throttledPlaceCaretAt = $_3l01gcijkmcwk99.throttle(function(
				clientX,
				clientY
			) {
				editor._selectionOverrides.hideFakeCaret();
			...
			return function(e) {
				...
				if (state.dragging) {
					var targetPos = applyRelPos(
						state,
						$_7vqqhl5qjkmcwlfd.calc(editor, e)
					);
			...
			editor.on('drop', function(e) {
				var realTarget =
					typeof e.clientX !== 'undefined'
						...
						: null;
				if (
					isContentEditableFalse$7(realTarget) ||
					isContentEditableFalse$7(
		...
		var walkUntil = function(
			...
			var add = function(node) {
				...
				for (i = 0; i < clientRects.length; i++) {
					...
					if (
						result.length > 0 &&
						isAboveFn(clientRect, $_9g1fibmjkmcwkbu.last(result))
					) {
						line++;
					}
			...
			targetClientRect = $_9g1fibmjkmcwkbu.last(
		...
		var positionsUntil = function(direction, root, predicateFn, node) {
			...
			do {
				...
				if (
					result.length > 0 &&
					isBelowFn(clientRect, $_9g1fibmjkmcwkbu.last(result))
				) {
					line++;
				}
				clientRect = clone$2(clientRect);
		...
		var findClosestClientRect = function(clientRects, clientX) {
			return $_9g1fibmjkmcwkbu.reduce(clientRects, function(
				oldClientRect,
				clientRect
			) {
				...
				if (
					...
					isContentEditableFalse$8(clientRect.node)
				) {
					return clientRect;
				}
		...
		var findLineNodeRects = function(root, targetNodeRect) {
			...
			var collect = function(checkPosFn, node) {
				var lineRects;
				lineRects = $_9g1fibmjkmcwkbu.filter(
					getClientRects([node]),
					function(clientRect) {
		...
		var getFakeCaretTargets = function(root) {
			return $_9g1fibmjkmcwkbu.filter(
				$_9g1fibmjkmcwkbu.toArray(root.getElementsByTagName('*')),
				isFakeCaretTarget
			);
		};
		...
		var closestCaret = function(root, clientX, clientY) {
			...
			var targetNodeRects = $_9g1fibmjkmcwkbu.filter(
		...
		var SelectionOverrides = function(editor) {
			...
			var registerEvents = function() {
				editor.on('mouseup', function(e) {
					...
					if (
						range$$1.collapsed &&
						$_8we2e850jkmcwlaw.isXYInContentArea(
							editor,
							e.clientX,
							e.clientY
						)
				...
				var hasNormalCaretPosition = function(elm) {
					...
					return (
						newPos &&
						!isBeforeContentEditableFalse$1(newPos) &&
						!isAfterContentEditableFalse$1(newPos)
					);
				};
				...
				var hasBetterMouseTarget = function(targetNode, caretNode) {
					...
					return (
						targetBlock &&
						!isInSameBlock$$1(targetBlock, caretBlock) &&
				...
				editor.on('mousedown', function(e) {
					...
					if (contentEditableRoot) {
						if (isContentEditableFalse$9(contentEditableRoot)) {
							...
							if (
								!(
									isContentEditableTrue$5(
										contentEditableRoot
									) && e.shiftKey
								) &&
								!$_95lcbc5ujkmcwlg1.isXYWithinRange(
									e.clientX,
									e.clientY,
									editor.selection.getRng()
								)
						...
						if (caretInfo) {
							if (
								!hasBetterMouseTarget(e.target, caretInfo.node)
							) {
								e.preventDefault();
								var range$$1 = showCaret$$1(
									1,
									caretInfo.node,
									caretInfo.before,
									false
				...
				editor.on('AfterSetSelectionRange', function(e) {
					...
					if (
						!isFakeSelectionElement(rng.startContainer.parentNode)
					) {
						removeContentEditableSelection();
					}
				});
			...
			var setContentEditableSelection = function(range$$1, forward) {
				...
				if (range$$1.collapsed) {
					if (!isRangeInCaretContainer(range$$1)) {
						if (forward === false) {
							caretPosition = getNormalizedRangeEndPoint$$1(
								-1,
								range$$1
							);
							if (
								isFakeCaretTarget(caretPosition.getNode(true))
							) {
								return showCaret$$1(
									-1,
				...
				$realSelectionContainer = $_4ki4d929jkmcwkmn
					.descendant(
						Element$$1.fromDom(editor.getBody()),
						'#' + realSelectionId
					)
		...
		function SaxParser(settings, schema) {
			...
			var parse = function(html) {
				...
				var decode = $_33qwr1sjkmcwkip.decode;
				var fixSelfClosing;
				var filteredUrlAttrs = $_4ujg4tljkmcwkbk.makeMap(
		...
		var read$3 = function(elm) {
			return $_9g1fibmjkmcwkbu.filter(
				$_9g1fibmjkmcwkbu.map(elm.childNodes, getOuterHtml),
				function(item) {
					return item.length > 0;
				}
			);
		...
		var write = function(fragments, elm) {
			var currentFragments = $_9g1fibmjkmcwkbu.map(
				elm.childNodes,
				getOuterHtml
			);
			applyDiff(
				$_3xze2m62jkmcwlh7.diff(currentFragments, fragments),
				elm
			);
			return elm;
		};
		...
		var createFromEditor = function(editor) {
			...
			trimmedFragments = bind(fragments, function(html) {
				var trimmed = $_2varuf5zjkmcwlgq.trimInternal(
					editor.serializer,
					html
				);
		...
		var getCleanLevelContent = function(level) {
			...
			each(
				$_2gqm0n27jkmcwkmi.descendants(elm, '*[data-mce-bogus]'),
				$_eualm21wjkmcwkku.unwrap
			);
		...
		function UndoManager(editor) {
			...
			editor.on('KeyUp', function(e) {
				...
				if (
					isFirstTypedCharacter &&
					self.typing &&
					$_3cqp0r5yjkmcwlgf.isEq(
						$_3cqp0r5yjkmcwlgf.createFromEditor(editor),
						data[0]
					) === false
				) {
			...
			self = {
				...
				beforeChange: function() {
					if (isUnlocked()) {
						beforeBookmark = $_aw3zi52jjkmcwkoa.getUndoBookmark(
							editor.selection
						);
					}
				},
				add: function(level, event) {
					...
					if (
						lastLevel &&
						$_3cqp0r5yjkmcwlgf.isEq(lastLevel, level)
					) {
						return null;
					}
					if (data[index]) {
					...
					level.bookmark = $_aw3zi52jjkmcwkoa.getUndoBookmark(
						editor.selection
					);
				...
				extra: function(callback1, callback2) {
					...
					if (self.transact(callback1)) {
						...
						$_3cqp0r5yjkmcwlgf.applyToEditor(
							editor,
							lastLevel,
							true
						);
					...
					for (key in items) {
						if (items.hasOwnProperty(key)) {
							if (itemName === 'attributes') {
								...
								value = $_dli08w3ajkmcwktg.getStyle(
									dom,
									node,
									key
								);
							...
							if (
								...
								!isEq$4(
									value,
									$_dli08w3ajkmcwktg.normalizeStyleValue(
										dom,
										$_dli08w3ajkmcwktg.replaceVars(
											items[key],
											vars
										),
										key
		...
		var split$1 = function(rng) {
			...
			if (
				...
				$_bjermr1rjkmcwkij.isText(startContainer)
			) {
				if (
					startOffset > 0 &&
				...
				if (
					$_bjermr1rjkmcwkij.isText(endContainer) &&
					endOffset > 0 &&
					endOffset < endContainer.nodeValue.length
				) {
		...
		var removeCaretContainerNode = function(
			dom,
			selection,
			node,
			moveCaret
		...
		var removeCaretContainer = function(
			body,
			dom,
			selection,
			node,
		...
		var insertCaretContainerNode = function(
			...
			var dom = editor.dom,
				block = dom.getParent(
					formatNode,
					$_fl2rax2ljkmcwkot.curry(
						$_dli08w3ajkmcwktg.isTextBlock,
						editor
					)
				);
			if (block && dom.isEmpty(block)) {
				...
				$_zygeg3ljkmcwkvo.removeTrailingBr(
					Element$$1.fromDom(formatNode)
				);
		...
		var applyCaretFormat = function(editor, name, vars) {
			...
			if (
				...
				rng.collapse(true);
				rng = $_97afwh2ujkmcwkpw.expandRng(
					editor,
					rng,
		...
		var removeCaretFormat = function(editor, name, vars, similar) {
			...
			if (hasContentAfter) {
				bookmark = selection.getBookmark();
				rng.collapse(true);
				var expandedRng = $_97afwh2ujkmcwkpw.expandRng(
					editor,
					rng,
					editor.formatter.get(name),
					true
		...
		var isFormatElement = function(editor, element) {
			...
			return (
				inlineElements.hasOwnProperty(
					$_2xwdjv11jkmcwke0.name(element)
				) &&
				!isCaretNode(element.dom()) &&
				!$_bjermr1rjkmcwkij.isBogus(element.dom())
			);
		};
		...
		addPostProcessHook('pre', function(editor) {
			...
			var hasPreSibling = function(pre) {
				return (
					...
					$_9g1fibmjkmcwkbu.indexOf(blocks, pre.previousSibling) !==
						-1
				);
			};
		...
		var getContainer = function(ed, rng, start) {
			...
			if (
				...
				container =
					new TreeWalker(container, ed.getBody()).next() || container;
			}
			if (
				$_bjermr1rjkmcwkij.isText(container) &&
				!start &&
				...
				container =
					new TreeWalker(container, ed.getBody()).prev() || container;
			}
			return container;
		};
			...
			if (format.block) {
				if (!forcedRootBlock) {
					if (dom.isBlock(node) && !dom.isBlock(parentNode)) {
						if (
							!find$4(dom, node, false) &&
							!find$4(dom, node.firstChild, true, 1)
						) {
							node.insertBefore(
								dom.create('br'),
								node.firstChild
						...
						if (
							!find$4(dom, node, true) &&
							!find$4(dom, node.lastChild, false, 1)
						) {
					...
					if (parentNode === dom.getRoot()) {
						if (
							...
							each$13(
								$_4ujg4tljkmcwkbk.grep(node.childNodes),
								function(node) {
									if (
										$_dli08w3ajkmcwktg.isValid(
											ed,
		...
		var removeFormat = function(ed, format, vars, node, compareNode) {
			...
			if (format.remove !== 'all') {
				each$13(format.styles, function(value, name) {
					value = $_dli08w3ajkmcwktg.normalizeStyleValue(
						dom,
						$_dli08w3ajkmcwktg.replaceVars(value, vars),
						name
					);
		...
		var findFormatRoot = function(editor, container, name, vars, similar) {
			...
			each$13(
				$_dli08w3ajkmcwktg
					.getParents(editor.dom, container.parentNode)
					.reverse(),
				function(parent) {
					var format;
					if (
						...
						format = $_axwnk168jkmcwlim.matchNode(
							editor,
							parent,
							name,
							vars,
		...
		var remove$8 = function(ed, name, vars, node, similar) {
				...
				if (format.split) {
					...
					if (startContainer !== endContainer) {
						...
						if (
							...
							startContainer !== endContainer &&
							!dom.isBlock(endContainer) &&
							!isTableCell$4(startContainer) &&
							!isTableCell$4(endContainer)
						) {
							startContainer = wrap$2(
								dom,
								startContainer,
								'span',
								{
						...
						startContainer = endContainer = splitToFormatRoot(
							startContainer
						);
					}
					...
					each$13(nodes, function(node) {
						...
						if (
							...
							ed.dom.getStyle(node, 'text-decoration') ===
								'underline' &&
							node.parentNode &&
							$_dli08w3ajkmcwktg.getTextDecoration(
								dom,
								node.parentNode
							) === 'underline'
						) {
			...
			if (
				...
				bookmark = $_aw3zi52jjkmcwkoa.getPersistentBookmark(
					ed.selection,
					true
				);
		...
		var isElementNode = function(node) {
			return (
				...
				!$_e4btcc2vjkmcwkq3.isBookmarkNode(node) &&
				!isCaretNode(node) &&
				!$_bjermr1rjkmcwkij.isBogus(node)
			);
		};
		var findElementSibling = function(node, siblingName) {
			...
			for (sibling = node; sibling; sibling = sibling[siblingName]) {
				...
				if (
					sibling.nodeType === 1 &&
					!$_e4btcc2vjkmcwkq3.isBookmarkNode(sibling)
				) {
					return sibling;
				}
			}
		...
		var mergeSiblingsNodes = function(dom, prev, next) {
			...
			if (prev && next) {
				...
				if (elementUtils.compare(prev, next)) {
					...
					dom.remove(next);
					$_4ujg4tljkmcwkbk.each(
						$_4ujg4tljkmcwkbk.grep(next.childNodes),
						function(node) {
		...
		var processUnderlineAndColor = function(dom, node) {
			...
			if (
				...
				textDecoration = $_dli08w3ajkmcwktg.getTextDecoration(
					dom,
					node.parentNode
				);
		...
		var mergeSiblings = function(dom, format, vars, node) {
			if (node && format.merge_siblings !== false) {
				node = mergeSiblingsNodes(
					dom,
					$_dli08w3ajkmcwktg.getNonWhiteSpaceSibling(node),
					node
				);
				node = mergeSiblingsNodes(
					dom,
					node,
					$_dli08w3ajkmcwktg.getNonWhiteSpaceSibling(node, true)
				);
			}
		};
		...
		var mergeWithChildren = function(editor, formatList, vars, node) {
			each$14(formatList, function(format) {
				each$14(editor.dom.select(format.inline, node), function(
					child
				) {
					if (!isElementNode(child)) {
						return;
					}
					$_2vklhc6cjkmcwlj5.removeFormat(
						editor,
						format,
						vars,
						child,
		...
		var mergeWithParents = function(editor, format, name, vars, node) {
			...
			if (format.merge_with_parents) {
				editor.dom.getParent(node.parentNode, function(parent) {
					if (
						$_axwnk168jkmcwlim.matchNode(editor, parent, name, vars)
					) {
						$_2vklhc6cjkmcwlj5.removeFormat(
							editor,
							format,
			...
			var applyRngStyle = function(dom, rng, bookmark, nodeSpecific) {
				...
				$_cuckrv3bjkmcwktk.walk(dom, rng, function(nodes) {
					...
					var process = function(node) {
						...
						if (
							format.wrapper &&
							$_axwnk168jkmcwlim.matchNode(ed, node, name, vars)
						) {
							currentWrapElm = 0;
							return;
						}
						if (
							...
							format.block &&
							!format.wrapper &&
							$_dli08w3ajkmcwktg.isTextBlock(ed, nodeName) &&
							$_dli08w3ajkmcwktg.isValid(ed, parentName, wrapName)
						) {
				...
				each$15(newWrappers, function(node) {
					...
					var getChildCount = function(node) {
						...
						each$15(node.childNodes, function(node) {
							if (
								!$_dli08w3ajkmcwktg.isWhiteSpaceNode(node) &&
								!$_e4btcc2vjkmcwkq3.isBookmarkNode(node)
							) {
								count++;
							}
					...
					var mergeStyles = function(node) {
						...
						if (
							child &&
							!$_e4btcc2vjkmcwkq3.isBookmarkNode(child) &&
							$_axwnk168jkmcwlim.matchName(dom, child, format)
						) {
					...
					if (format.inline || format.wrapper) {
						...
						$_ehzows6bjkmcwliw.mergeWithChildren(
							ed,
							formatList,
							vars,
							node
						);
						$_ehzows6bjkmcwliw.mergeWithParents(
							ed,
							format,
							name,
							vars,
						...
						$_ehzows6bjkmcwliw.mergeBackgroundColorAndFontSize(
							dom,
							format,
							vars,
							node
						...
						$_ehzows6bjkmcwliw.mergeSiblings(
							dom,
							format,
							vars,
							node
			...
			if (format) {
				if (node) {
					...
					if (
						...
						ed.selection.setRng(
							$_bx30le3ojkmcwkw9.normalize(ed.selection.getRng())
						);
						bookmark = $_aw3zi52jjkmcwkoa.getPersistentBookmark(
							ed.selection,
							true
						...
						if (format.styles) {
							$_ehzows6bjkmcwliw.mergeUnderlineAndColor(
								dom,
								format,
								vars,
								curSelNode
		...
		var setup$5 = function(formatChangeData, editor) {
			...
			editor.on('NodeChange', function(e) {
				var parents = $_dli08w3ajkmcwktg.getParents(
					editor.dom,
					e.element
				);
				var matchedFormats = {};
				...
				each$16(formatChangeData.get(), function(callbacks, format) {
					each$16(parents, function(node) {
						...
						if (
							$_axwnk168jkmcwlim.matchesUnInheritedFormatSelector(
								editor,
								node,
								format
							)
		...
		var parseSelector = function(selector) {
			...
			return $_4ujg4tljkmcwkbk
				.map(selector.split(/(?:>|\s+(?![^\[\]]+\]))/), function(item) {
					var siblings = $_4ujg4tljkmcwkbk.map(
						item.split(/(?:~\+|~|\+)/),
						parseSelectorItem
					);
			...
			each$17(previewStyles.split(' '), function(name) {
				...
				if (name === 'font-size') {
					if (/em|%$/.test(value)) {
						...
						var numValue =
							parseFloat(value) / (/%$/.test(value) ? 100 : 1);
		...
		var toggle$3 = function(editor, formats, name, vars, node) {
			...
			if (
				$_axwnk168jkmcwlim.match(editor, name, vars, node) &&
		...
		function Formatter(editor) {
			...
			return {
				...
				formatChanged: curry(
					$_5j3j9s6djkmcwljg.formatChanged,
					editor,
					formatChangeState
				),
		...
		var register = function(htmlParser, settings, dom) {
			htmlParser.addAttributeFilter('data-mce-tabindex', function(
				nodes,
				name
			) {
				var i = nodes.length,
			...
			htmlParser.addAttributeFilter('src,href,style', function(
				nodes,
				name
			) {
				var i = nodes.length,
			...
			htmlParser.addAttributeFilter('data-mce-type', function(
				nodes,
				name,
				args
			) {
			...
			htmlParser.addNodeFilter('xml:namespace,input', function(
				nodes,
				name
			) {
				var i = nodes.length,
		...
		var preProcess = function(editor, node, args) {
			...
			$_q3ric4tjkmcwl9q.firePreProcess(
				editor,
				merge(args, { node: node })
			);
			if (oldDoc) {
				dom.doc = oldDoc;
			}
		...
		var addFontToSpansFilter = function(domParser, styles, fontSizes) {
			domParser.addNodeFilter('font', function(nodes) {
				each(nodes, function(node) {
					...
					if (size) {
						props['font-size'] =
							fontSizes[parseInt(node.attr('size'), 10) - 1];
					}
					node.name = 'span';
		...
		var addFilters = function(domParser, settings) {
			...
			if (settings.convert_fonts_to_spans) {
				addFontToSpansFilter(
					domParser,
					styles,
					$_4ujg4tljkmcwkbk.explode(settings.font_size_legacy_values)
				);
			}
		...
		var isPaddedWithNbsp = function(node) {
			return (
				hasOnlyChild(node, '#text') && node.firstChild.value === '\xA0'
			);
		};
		...
		var isEmpty$2 = function(
			...
			return node.isEmpty(nonEmptyElements, whitespaceElements, function(
				node
			) {
		...
		var register$2 = function(parser, settings) {
			...
			if (settings.remove_trailing_brs) {
				parser.addNodeFilter('br', function(nodes, _, args) {
					...
					var l = nodes.length;
					var node;
					var blockElements = $_4ujg4tljkmcwkbk.extend(
						{},
				...
				for (ni = 0; ni < nodes.length; ni++) {
					...
					if (parent && parents.length > 1) {
						...
						if (node.name === 'li') {
							...
							if (
								sibling &&
								(sibling.name === 'ul' || sibling.name === 'ul')
							) {
								sibling.append(node);
								continue;
							}
		...
		var postProcess$1 = function(editor, args, content) {
			if (!args.no_events && editor) {
				var outArgs = $_q3ric4tjkmcwl9q.firePostProcess(
					editor,
		...
		var getHtmlFromNode = function(dom, node, args) {
			var html = $_cadvcr2pjkmcwkph.trim(
				args.getInner ? node.innerHTML : dom.getOuterHTML(node)
			);
			return args.selection ||
				isWsPreserveElement(Element$$1.fromDom(node))
				? html
				: $_4ujg4tljkmcwkbk.trim(html);
		};
		...
		function BookmarkManager(selection) {
			return {
				...
				moveToBookmark: curry(
					$_e4btcc2vjkmcwkq3.moveToBookmark,
					selection
				)
			};
		}
		...
		var isContentEditableFalse$10 =
			$_bjermr1rjkmcwkij.isContentEditableFalse;
		var isContentEditableTrue$6 = $_bjermr1rjkmcwkij.isContentEditableTrue;
		var getContentEditableRoot$2 = function(root, node) {
			while (node && node !== root) {
				if (
					isContentEditableTrue$6(node) ||
					isContentEditableFalse$10(node)
				) {
					return node;
				}
				node = node.parentNode;
		...
		var ControlSelection = function(selection, editor) {
			...
			var resizeGhostElement = function(e) {
				...
				if (
					...
					proportional =
						$_6bj5795wjkmcwlg6.modifierPressed(e) ||
						(isImage(selectedElm) &&
				...
				if (!resizeStarted) {
					$_q3ric4tjkmcwl9q.fireObjectResizeStart(
						editor,
						selectedElm,
						startW,
						startH
			...
			var endGhostResize = function() {
				...
				$_q3ric4tjkmcwl9q.fireObjectResized(
					editor,
					selectedElm,
					width,
					height
			...
			var isWithinContentEditableFalse = function(elm) {
				return isContentEditableFalse$10(
			...
			editor.on('init', function() {
				...
				if ($_cvvl9iajkmcwk25.ie && $_cvvl9iajkmcwk25.ie >= 11) {
					...
					editor.dom.bind(rootElement, 'mscontrolselect', function(
						e
					) {
						var delayedSelect = function(node) {
							$_3l01gcijkmcwk99.setEditorTimeout(
								editor,
								function() {
				...
				var throttledUpdateResizeRect = $_3l01gcijkmcwk99.throttle(
					function(e) {
						if (!editor.composing) {
							updateResizeRect(e);
						}
		...
		var moveOutOfContentEditableFalse = function(rng, rootNode) {
			...
			return $_bjermr1rjkmcwkij.isContentEditableFalse(
		...
		var extractRows = function(table, sx, sy, ex, ey) {
			...
			for (var y = sy; y <= ey; y++) {
				var cells = rows[y].cells();
				var slice =
					sx < ex ? cells.slice(sx, ex + 1) : cells.slice(ex, sx + 1);
		...
		var fromDom$2 = function(tableElm) {
			...
			each($_2gqm0n27jkmcwkmi.descendants(tableElm, 'tr'), function(
				tr,
				y
			) {
				each($_2gqm0n27jkmcwkmi.descendants(tr, 'td,th'), function(
					td,
					x
				) {
		...
		var findParentListContainer = function(parents) {
			return find(parents, function(elm) {
				return (
					$_2xwdjv11jkmcwke0.name(elm) === 'ul' ||
					$_2xwdjv11jkmcwke0.name(elm) === 'ol'
				);
			});
		};
		var getFullySelectedListWrappers = function(parents, rng) {
			return find(parents, function(elm) {
				return (
					$_2xwdjv11jkmcwke0.name(elm) === 'li' &&
					hasAllContentsSelected(elm, rng)
				);
				...
				return findParentListContainer(parents)
					.map(function(listCont) {
						return [
							...
							Element$$1.fromTag(
								$_2xwdjv11jkmcwke0.name(listCont)
							)
						];
					})
					.getOr([]);
		...
		var wrap$3 = function(innerElm, elms) {
			...
			return elms.length > 0
				? $_a3l7fy70jkmcwloa.fromElements([wrapped])
				: wrapped;
		};
		var directListWrappers = function(commonAnchorContainer) {
			if (isListItem(commonAnchorContainer)) {
				return $_ca6pc118jkmcwkfs
					.parent(commonAnchorContainer)
					.filter(isList)
		...
		var getWrapElements = function(rootNode, rng) {
			var commonAnchorContainer = Element$$1.fromDom(
				rng.commonAncestorContainer
			);
			var parents = $_cir6zl3zjkmcwl2j.parentsAndSelf(
				commonAnchorContainer,
				rootNode
			);
		...
		var getFragmentFromRange = function(rootNode, rng) {
			return wrap$3(
				Element$$1.fromDom(rng.cloneContents()),
				getWrapElements(rootNode, rng)
			);
		};
		var getParentTable = function(rootElm, cell) {
			return $_4ki4d929jkmcwkmn.ancestor(
				cell,
				'table',
				curry($_e4hu6h1ejkmcwkgj.eq, rootElm)
			);
		};
		var getTableFragment = function(rootNode, selectedTableCells) {
			return getParentTable(rootNode, selectedTableCells[0])
				.bind(function(tableElm) {
					...
					return $_6wuzo771jkmcwlof
						.subsection(fullTableModel, firstCell, lastCell)
						.map(function(sectionedTableModel) {
							return $_a3l7fy70jkmcwloa.fromElements([
								$_6wuzo771jkmcwlof.toDom(sectionedTableModel)
							]);
						});
				})
		...
		var read$4 = function(rootNode, ranges) {
			var selectedCells = $_7q5gk44ejkmcwl6p.getCellsFromElementOrRanges(
				ranges,
				rootNode
			);
			return selectedCells.length > 0
		...
		var getContent = function(editor, args) {
			...
			var sel = editor.selection.getSel();
			var fragment;
			var ranges = $_8161126xjkmcwlnd.processRanges(
				editor,
				$_admly54fjkmcwl6u.getRanges(sel)
			);
			args = args || {};
			args.get = true;
			...
			if (args.format === 'text') {
				return editor.selection.isCollapsed()
					? ''
					: $_cadvcr2pjkmcwkph.trim(
							rng.text || (sel.toString ? sel.toString() : '')
					  );
			}
			if (rng.cloneContents) {
				fragment = args.contextual
					? $_2bm266zjkmcwlnj
							.read(Element$$1.fromDom(editor.getBody()), ranges)
							.dom()
					: rng.cloneContents();
				if (fragment) {
			...
			return Option.from(container)
				...
				.map(function(elm) {
					return !real || !rng.collapsed
						? $_ca6pc118jkmcwkfs
								.child(elm, resolve(elm, offset))
								.getOr(elm)
						: elm;
				})
				.bind(function(elm) {
					return $_2xwdjv11jkmcwke0.isElement(elm)
						? Option.some(elm)
						: $_ca6pc118jkmcwkfs.parent(elm);
				})
				.map(function(elm) {
					return elm.dom();
				})
		...
		var getStart$2 = function(root, rng, real) {
			return getEndpointElement(root, rng, true, real, function(
				elm,
				offset
			) {
				return Math.min(
					$_ca6pc118jkmcwkfs.childNodesCount(elm),
					offset
				);
			});
		};
		var getEnd = function(root, rng, real) {
			return getEndpointElement(root, rng, false, real, function(
				elm,
				offset
			) {
		...
		var isAttachedToDom = function(node) {
			return (
				...
				$_e4hu6h1ejkmcwkgj.contains(
					Element$$1.fromDom(node.ownerDocument),
					Element$$1.fromDom(node)
				)
			);
		};
		...
		var Selection$1 = function(dom, win, serializer, editor) {
			...
			var getRng = function() {
				...
				if (
					...
					$_e3v8sn4vjkmcwl9y.hasFocus(editor) === false
				) {
					var bookmark = $_39c3zs4ljkmcwl86.getRng(editor);
					if (bookmark.isSome()) {
						return bookmark
							.map(function(r) {
								return $_8161126xjkmcwlnd.processRanges(
									editor,
									[r]
								)[0];
							})
			...
			var normalize = function() {
				...
				var sel = getSel();
				if (
					!$_admly54fjkmcwl6u.hasMultipleRanges(sel) &&
					hasAnyRanges(editor)
				) {
				...
				if (!selectorChangedData) {
					...
					editor.on('NodeChange', function(e) {
						var node = e.element,
							...
							matchedSelectors = {};
						each$19(selectorChangedData, function(
							callbacks,
							selector
						) {
						...
						each$19(currentSelectors, function(
							callbacks,
							selector
						) {
			...
			var scrollIntoView = function(elm, alignToTop) {
				return $_a6610x37jkmcwkt5.scrollElementIntoView(
					editor,
					elm,
					alignToTop
				);
			...
			var getBoundingClientRect = function() {
				var rng = getRng();
				return rng.collapsed
					? CaretPosition$1.fromRangeStart(rng).getClientRects()[0]
		...
		var moveToCeFalseHorizontally = function(
			...
			var isBeforeContentEditableFalseFn = forwards
				? isBeforeContentEditableFalse$2
				: isAfterContentEditableFalse$2;
			if (!range.collapsed) {
		...
		var moveToCeFalseVertically = function(
			...
			nextLinePositions = $_9g1fibmjkmcwkbu.filter(
				linePositions,
				isLine(1)
			);
			caretClientRect = $_9g1fibmjkmcwkbu.last(
				caretPosition.getClientRects()
			);
			if (
				isBeforeContentEditableFalse$2(caretPosition) ||
				isBeforeTable(caretPosition)
			) {
				contentEditableFalseNode = caretPosition.getNode();
			}
			if (
				isAfterContentEditableFalse$2(caretPosition) ||
			...
			if (contentEditableFalseNode) {
				...
				closestNextLineRect = findClosestClientRect(
					$_9g1fibmjkmcwkbu.filter(caretPositions, isLine(1)),
					clientX
				);
				if (closestNextLineRect) {
				...
				closestNextLineRect = $_9g1fibmjkmcwkbu.last(
					$_9g1fibmjkmcwkbu.filter(caretPositions, isLine(0))
				);
				if (closestNextLineRect) {
					return renderRangeCaret(
						editor,
		...
		var exitPreBlock = function(editor, direction, range) {
			...
			var caretWalker = CaretWalker(editor.getBody());
			var getNextVisualCaretPosition = $_fl2rax2ljkmcwkot.curry(
				getVisualCaretPosition,
				caretWalker.next
			);
			var getPrevVisualCaretPosition = $_fl2rax2ljkmcwkot.curry(
				getVisualCaretPosition,
				caretWalker.prev
			);
				...
				.execute(
					[
						...
						},
						{
							...
							ctrlKey: !os.isOSX(),
							altKey: os.isOSX(),
							action: $_dl6t9m4ajkmcwl5b.moveNextWord(
								editor,
								caret
							)
						},
						{
							...
							ctrlKey: !os.isOSX(),
							altKey: os.isOSX(),
							action: $_dl6t9m4ajkmcwl5b.movePrevWord(
								editor,
								caret
							)
						}
		...
		var getParentInlines = function(rootElm, startElm) {
			...
			return findIndex(parents, isBlock).fold(constant(parents), function(
				index
			) {
		...
		var deleteCaret$1 = function(editor, forward) {
			...
			return last(parentInlines)
				.map(function(target) {
					...
					if (
						$_8z75uj3sjkmcwkx3.willDeleteLastPositionInElement(
							forward,
							fromPos,
							target.dom()
						)
					...
					[
						{
							...
							action: $_3olmgr77jkmcwlpj.action(
								$_1hyiiy41jkmcwl2z.backspaceDelete,
								editor,
								false
							)
						},
						...
						},
						{
							keyCode: $_6bj5795wjkmcwlg6.BACKSPACE,
							action: $_3olmgr77jkmcwlpj.action(
								$_dqvreg45jkmcwl4e.backspaceDelete,
								editor,
								caret,
								false
							)
						...
						},
						{
							keyCode: $_6bj5795wjkmcwlg6.BACKSPACE,
							action: $_3olmgr77jkmcwlpj.action(
								$_5lrlvq4cjkmcwl5t.backspaceDelete,
								editor,
								false
							)
						},
						...
						},
						{
							keyCode: $_6bj5795wjkmcwlg6.BACKSPACE,
							action: $_3olmgr77jkmcwlpj.action(
								$_egxtcx40jkmcwl2q.backspaceDelete,
								editor,
								false
							)
						},
						...
						},
						{
							keyCode: $_6bj5795wjkmcwlg6.BACKSPACE,
							action: $_3olmgr77jkmcwlpj.action(
								$_1wrlyf3qjkmcwkwe.backspaceDelete,
								editor,
								false
							)
						},
						...
						},
						{
							keyCode: $_6bj5795wjkmcwlg6.BACKSPACE,
							action: $_3olmgr77jkmcwlpj.action(
								$_7tr96779jkmcwlpz.backspaceDelete,
								editor,
								false
							)
						},
				...
				[
					{
						keyCode: $_6bj5795wjkmcwlg6.BACKSPACE,
						action: $_3olmgr77jkmcwlpj.action(
							$_1hyiiy41jkmcwl2z.paddEmptyElement,
							editor
						)
					},
					{
		...
		var getParentBlockName = function(editor) {
			return getParentBlock$2(editor).fold(constant(''), function(
				parentBlock
			) {
		...
		var isEmptyAnchor = function(elm) {
			return (
				elm &&
				elm.nodeName === 'A' &&
				$_4ujg4tljkmcwkbk.trim(
					$_cadvcr2pjkmcwkph.trim(elm.innerText || elm.textContent)
				).length === 0
			);
		};
		...
		var normalizeZwspOffset = function(start, container, offset) {
			if ($_bjermr1rjkmcwkij.isText(container) === false) {
				...
				return offset === container.data.length - 1 &&
					container.data.charAt(offset) === $_cadvcr2pjkmcwkph.ZWSP
					? container.data.length
					: offset;
			}
		};
		...
		var setForcedBlockAttrs = function(editor, node) {
			var forcedRootBlockName = $_2ycm1438jkmcwkta.getForcedRootBlock(
				editor
			);
			if (
				forcedRootBlockName &&
		...
		var insert$2 = function(editor, evt) {
			...
			var insertNewBlockAfter = function() {
				...
				if (
					$_2ycm1438jkmcwkta.shouldEndContainerOnEmptyBlock(editor) &&
			...
			if (isCaretContainerBlock(parentBlock)) {
				...
				$_79nvm87ejkmcwlqu.moveToCaretPosition(
					editor,
		...
		var matchesSelector = function(editor, selector) {
			return $_79nvm87ejkmcwlqu
				.getParentBlock(editor)
				.filter(function(parentBlock) {
					return (
						selector.length > 0 &&
						$_4jv6d81gjkmcwkgv.is(
							Element$$1.fromDom(parentBlock),
							selector
						)
					);
				})
		...
		var shouldInsertBr = function(editor) {
			return matchesSelector(
				editor,
				$_2ycm1438jkmcwkta.getBrNewLineSelector(editor)
			);
		};
		var shouldBlockNewLine = function(editor) {
			return matchesSelector(
				editor,
				$_2ycm1438jkmcwkta.getNoNewLineSelector(editor)
			);
		};
		...
		var isBrMode = function(requiredState) {
			return function(editor, shiftKey) {
				var brMode =
					$_2ycm1438jkmcwkta.getForcedRootBlock(editor) === '';
		...
		var inListBlock = function(requiredState) {
			return function(editor, shiftKey) {
				return (
					$_79nvm87ejkmcwlqu.isListItemParentBlock(editor) ===
					requiredState
				);
			};
		};
		var inBlock = function(blockName, requiredState) {
			return function(editor, shiftKey) {
				var state =
					$_79nvm87ejkmcwlqu.getParentBlockName(editor) ===
		...
		var shouldPutBrInPre$1 = function(requiredState) {
			return function(editor, shiftKey) {
				return (
					$_2ycm1438jkmcwkta.shouldPutBrInPre(editor) ===
					requiredState
				);
			};
		};
		...
		var canInsertIntoEditableRoot = function(editor) {
			var forcedRootBlock = $_2ycm1438jkmcwkta.getForcedRootBlock(editor);
			var rootEditable = $_79nvm87ejkmcwlqu.getEditableRoot(
				editor.dom,
				editor.selection.getStart()
			);
			return (
		...
		var insert$3 = function(editor, evt) {
			$_47r1sh7fjkmcwlr1.getAction(editor, evt).fold(
				function() {
		...
		var isValidInsertPoint = function(location, caretPosition) {
			return (
				isAtStartOrEnd(location) &&
				$_bjermr1rjkmcwkij.isText(caretPosition.container())
			);
		};
		...
		var insertAtCaret$2 = function(editor) {
			...
			var boundaryLocation = $_gau9ep48jkmcwl4x.readLocation(
				isInlineTarget,
				editor.getBody(),
				caretPosition
			);
			return boundaryLocation
				.map(curry(insertAtLocation, editor, caretPosition))
				.getOr(false);
		};
					...
					[
						{
							...
							action: $_3olmgr77jkmcwlpj.action(
								$_2cwo047ijkmcwlrj.insertAtSelection,
								editor
							)
						}
					],
		...
		function Quirks(editor) {
			...
			var BACKSPACE = $_6bj5795wjkmcwlg6.BACKSPACE,
				DELETE = $_6bj5795wjkmcwlg6.DELETE,
				dom = editor.dom,
				selection = editor.selection,
				settings = editor.settings,
				parser = editor.parser;
			var isGecko = $_cvvl9iajkmcwk25.gecko,
			...
			var getMceInternalContent = function(e) {
				var internalContent;
				if (e.dataTransfer) {
					internalContent = e.dataTransfer.getData(
						mceInternalDataType
					);
					if (
						internalContent &&
			...
			var removeHrOnBackspace = function() {
				editor.on('keydown', function(e) {
					if (!isDefaultPrevented(e) && e.keyCode === BACKSPACE) {
						if (
							!editor.getBody().getElementsByTagName('hr').length
						) {
							return;
						}
						if (
				...
				editor.on('keydown', function(e) {
					if (!isDefaultPrevented(e) && e.keyCode === BACKSPACE) {
						if (
							...
							var previousSibling = selection.getNode()
								.previousSibling;
							if (
								previousSibling &&
								previousSibling.nodeName &&
			...
			var removeBlockQuoteOnBackSpace = function() {
				editor.on('keydown', function(e) {
					...
					if (
						isDefaultPrevented(e) ||
						e.keyCode !== $_6bj5795wjkmcwlg6.BACKSPACE
					) {
						return;
					}
					rng = selection.getRng();
					...
					while (
						parent$$1 &&
						parent$$1.parentNode &&
						parent$$1.parentNode.firstChild === parent$$1 &&
						parent$$1.parentNode !== root
					) {
			...
			var blockCmdArrowNavigation = function() {
				if ($_cvvl9iajkmcwk25.mac) {
					editor.on('keydown', function(e) {
						if (
							$_6bj5795wjkmcwlg6.metaKeyPressed(e) &&
							!e.shiftKey &&
				...
				editor.on('drop', function(e) {
					if (!isDefaultPrevented(e)) {
						...
						if (
							...
							e.preventDefault();
							var rng = $_52q21w6wjkmcwlna.fromPoint(
								e.x,
								e.y,
								editor.getDoc()
							);
		...
		var isTextBlockNode = function(node) {
			return (
				$_bjermr1rjkmcwkij.isElement(node) &&
				isTextBlock(Element$$1.fromDom(node))
			);
		};
		...
		var createParser = function(editor) {
			...
			parser.addAttributeFilter('src,href,style,tabindex', function(
				nodes,
				name$$1
			) {
				var i = nodes.length,
				...
				while (i--) {
					...
					if (!node.attributes.map[internalName]) {
						...
						if (name$$1 === 'style') {
							...
							node.attr(
								internalName,
								editor.convertURL(value, name$$1, node.name)
							);
						}
					}
				}
		...
		var initContentBody = function(editor, skipWrite) {
			...
			if (settings.content_editable) {
				...
				editor.contentDocument = doc =
					settings.content_document || document;
		...
		var getIframeHtml = function(editor) {
			...
			if (
				$_2ycm1438jkmcwkta.getDocumentBaseUrl(editor) !==
			...
			if ($_2ycm1438jkmcwkta.getContentSecurityPolicy(editor)) {
				iframeHTML +=
					...
					$_2ycm1438jkmcwkta.getContentSecurityPolicy(editor) +
					'" />';
			}
			iframeHTML +=
				'</head><body id="' +
		...
		var createIframe = function(editor, o) {
			...
			var ifr = createIframeElement(
				...
				title,
				o.height,
				$_2ycm1438jkmcwkta.getIframeAttrs(editor)
			).dom();
			ifr.onload = function() {
				ifr.onload = null;
				editor.fire('load');
		...
		var initPlugin = function(editor, initializedPlugins, plugin) {
			...
			if (
				Plugin &&
				$_4ujg4tljkmcwkbk.inArray(initializedPlugins, plugin) === -1
			) {
				$_4ujg4tljkmcwkbk.each(
		...
		var init$2 = function(editor) {
			...
			if (settings.content_css) {
				$_4ujg4tljkmcwkbk.each(
					$_4ujg4tljkmcwkbk.explode(settings.content_css),
					function(u) {
		...
		var loadPlugins = function(settings, suffix) {
			...
			$_4ujg4tljkmcwkbk.each(settings.external_plugins, function(
				url,
				name$$1
			) {
			...
			$_4ujg4tljkmcwkbk.each(settings.plugins.split(/[ ,]/), function(
				plugin
			) {
				plugin = $_4ujg4tljkmcwkbk.trim(plugin);
			...
			if (!/^[\w\-]*:?\/\//.test(url)) {
				...
				if (settings.base_uri.protocol == '') {
					...
					url =
						...
						'://mce_host' +
						self$$1.toAbsPath(baseUrl, url[1]) +
						url[2];
				}
			}
		...
		URI.prototype = {
			...
			toRelative: function(uri) {
				...
				if (
					(uri.host !== 'mce_host' &&
						self$$1.host !== uri.host &&
						uri.host) ||
					self$$1.port !== uri.port ||
					(self$$1.protocol !== uri.protocol && uri.protocol !== '')
				) {
					return uri.getURI();
				}
				var tu = self$$1.getURI(),
					uu = uri.getURI();
				if (
					tu === uu ||
		...
		var getContentFromBody = function(editor, args, body) {
			...
			if (args.format === 'raw') {
				content = $_4ujg4tljkmcwkbk.trim(
					$_2varuf5zjkmcwlgq.trimExternal(
						editor.serializer,
						body.innerHTML
					)
				);
			} else if (args.format === 'text') {
				content = $_cadvcr2pjkmcwkph.trim(
			...
			if (
				args.format !== 'text' &&
				!isWsPreserveElement(Element$$1.fromDom(body))
			) {
		...
		var getContent$1 = function(editor, args) {
			...
			return Option.from(editor.getBody()).fold(
				constant(args.format === 'tree' ? new Node$2('body', 11) : ''),
				function(body) {
			...
			if ($_e3v8sn4vjkmcwl9y.hasFocus(editor)) {
				$_1m3f3j2yjkmcwkqg
					...
					.each(function(pos) {
						...
						var caretPos = $_bjermr1rjkmcwkij.isTable(node)
							? $_1m3f3j2yjkmcwkqg
									.firstPositionIn(node)
									.getOr(pos)
							: pos;
		...
		var setContentString = function(editor, body, content, args) {
			...
			if (content.length === 0 || /^\s+$/.test(content)) {
				...
				forcedRootBlockName = $_2ycm1438jkmcwkta.getForcedRootBlock(
					editor
				);
				if (
					forcedRootBlockName &&
				...
				args.content = isWsPreserveElement(Element$$1.fromDom(body))
					? content
					: $_4ujg4tljkmcwkbk.trim(content);
		...
		var registerEvents = function(editorManager, e) {
			...
			editor.on('focusout', function() {
				...
				$_3l01gcijkmcwk99.setEditorTimeout(self$$1, function() {
					var focusedEditor = editorManager.focusedEditor;
					if (
						!isUIElement(self$$1, getActiveElement()) &&
						focusedEditor === self$$1
					) {
			...
			translate: function(text) {
				...
				var isEmpty = function(text) {
					return (
						text === '' ||
						text === null ||
						$_4ujg4tljkmcwkbk.is(text, 'undefined')
					);
				};
				var getLangData = function(text) {
					text = toString(text);
					return $_4ujg4tljkmcwkbk.hasOwn(langData, text)
				...
				if (
					$_4ujg4tljkmcwkbk.is(text, 'object') &&
					$_4ujg4tljkmcwkbk.hasOwn(text, 'raw')
				) {
				...
				if ($_4ujg4tljkmcwkbk.is(text, 'array')) {
					...
					text = getLangData(text[0]).replace(
						/\{([0-9]+)\}/g,
						function($1, $2) {
							return $_4ujg4tljkmcwkbk.hasOwn(values_1, $2)
								? toString(values_1[$2])
								: $1;
						}
					);
		...
		EditorManager = {
			...
			overrideDefaults: function(defaultSettings) {
				...
				for (var name$$1 in pluginBaseUrls) {
					AddOnManager.PluginManager.urls[name$$1] =
						pluginBaseUrls[name$$1];
				}
			},
			init: function(settings) {
				var self$$1 = this;
				var result, invalidInlineTargets;
				invalidInlineTargets = $_4ujg4tljkmcwkbk.makeMap(
		...
		function RangeUtils(dom) {
			...
			var normalize = function(rng) {
				return $_21c07i4jjkmcwl7w
					.normalize(dom, rng)
					.fold(constant(false), function(normalizedRng) {
						rng.setStart(
