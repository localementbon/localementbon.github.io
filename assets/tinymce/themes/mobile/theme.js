	...
	var mobile = (function() {
		...
		var detectBrowser = function(browsers, userAgent) {
			return detect$1(browsers, userAgent).map(function(browser) {
				var version = $_9006f6y7jkmcwti0.detect(
		...
		var detect$2 = function(userAgent) {
			...
			var browser = $_8rokcbyajkmcwti8
				.detectBrowser(browsers, userAgent)
				.fold($_388mcgy6jkmcwthw.unknown, $_388mcgy6jkmcwthw.nu);
			var os = $_8rokcbyajkmcwti8
				.detectOs(oses, userAgent)
				.fold($_bik5l8y8jkmcwti1.unknown, $_bik5l8y8jkmcwti1.nu);
		...
		var tap = alloy.tap;
		var tapOrClick = $_b90373y3jkmcwths.detect().deviceType.isTouch()
			? alloy.tap
			: click;
		...
		var isComment = function(element) {
			return (
				type(element) === $_evhzkgyljkmcwtk0.COMMENT ||
		...
		var inBody = function(element) {
			var dom = $_b0ubstykjkmcwtjz.isText(element)
		...
		var ieContains = function(e1, e2) {
			return $_d18qkiyujkmcwtl5.documentPositionContainedBy(
				e1.dom(),
				e2.dom()
			);
		};
		...
		var appendAt = function(parent, element, index) {
			$_a7cj16ynjkmcwtk5.child(parent, index).fold(
				function() {
					append(parent, element);
				},
				function(v) {
		...
		var formatObj = function(input) {
			return isObject(input) && keys(input).length > 100
				? ' removed due to size'
				: $_ccklnrzhjkmcwton.stringify(input, null, 2);
		};
			...
			if (
				...
				throw new Error(
					'EventHandler defined by: ' +
						$_ccklnrzhjkmcwton.stringify(parts, null, 2) +
		...
		var doCreate = function(
			...
			var me = deepMerge(wrappedExtra, wrappedApis, {
				...
				handlers: function(info) {
					return getConfig(info)
						.bind(function(behaviourInfo) {
							return readOptFrom$1(active, 'events').map(function(
								events
							) {
								return events(
									behaviourInfo.config,
		...
		var transfer = function(source, destination, attrs) {
			if (
				!$_b0ubstykjkmcwtjz.isElement(source) ||
				!$_b0ubstykjkmcwtjz.isElement(destination)
			)
				return;
		...
		var remove$2 = function(element, attr, id) {
			...
			if (nu.length > 0)
				$_ga5avvzwjkmcwtro.set(element, attr, nu.join(' '));
			else $_ga5avvzwjkmcwtro.remove(element, attr);
			return false;
		};
		...
		var add$2 = function(element, clazz) {
			...
			else $_7te3evzxjkmcwtrx.add(element, clazz);
		};
		var cleanClass = function(element) {
			var classList = $_7te3evzxjkmcwtrx.supports(element)
				? element.dom().classList
				: $_7te3evzxjkmcwtrx.get(element);
		...
		var remove$4 = function(element, clazz) {
			...
			} else $_7te3evzxjkmcwtrx.remove(element, clazz);
			cleanClass(element);
		};
		var toggle$1 = function(element, clazz) {
			return $_7te3evzxjkmcwtrx.supports(element)
				? element.dom().classList.toggle(clazz)
				: $_7te3evzxjkmcwtrx.toggle(element, clazz);
		};
		...
		var has$1 = function(element, clazz) {
			return (
				$_7te3evzxjkmcwtrx.supports(element) &&
		...
		var focusInside = function(element) {
			...
			var inside = active(doc).filter(function(a) {
				return $_4o5r1x102jkmcwtsi.closest(
					a,
					curry($_cpo6bcytjkmcwtkv.eq, element)
				);
			});
		...
		var search = function(element) {
			return active($_a7cj16ynjkmcwtk5.owner(element)).filter(function(
				e
			) {
		...
		var orientation = function(onReceive) {
			return Receiving.config({
				channels: wrap$2($_5zff6b109jkmcwtt8.orientationChanged(), {
					onReceive: onReceive
				})
			});
		};
		...
		var get$3 = function(element, property) {
			...
			var v =
				r === '' && !$_3wo4fwyhjkmcwtje.inBody(element)
					? getUnsafeProperty(dom, property)
					: r;
			return v === null ? undefined : v;
		};
		var getUnsafeProperty = function(dom, property) {
			return $_88h3un117jkmcwtza.isSupported(dom)
		...
		var remove$5 = function(element, property) {
			...
			if (
				$_ga5avvzwjkmcwtro.has(element, 'style') &&
				trim($_ga5avvzwjkmcwtro.get(element, 'style')) === ''
			) {
		...
		var copy$1 = function(source, target) {
			...
			if (
				$_88h3un117jkmcwtza.isSupported(sourceDom) &&
				$_88h3un117jkmcwtza.isSupported(targetDom)
			) {
		...
		var api = Dimension('height', function(element) {
			var dom = element.dom();
			return $_3wo4fwyhjkmcwtje.inBody(element)
		...
		var ancestors$1 = function(scope, selector, isRoot) {
			return $_cpeyp611ajkmcwtzi.ancestors(
				scope,
				function(e) {
		...
		var ancestor$2 = function(scope, selector, isRoot) {
			return $_exwlcm103jkmcwtsk.ancestor(
				scope,
				function(e) {
					return $_2lpyciyyjkmcwtlc.is(e, selector);
				},
		...
		var dehighlightAll = function(component, hConfig, hState) {
			var highlighted = $_bpsi2e119jkmcwtzh.descendants(
				component.element(),
		...
		var highlightBy = function(component, hConfig, hState, predicate) {
			var items = $_bpsi2e119jkmcwtzh.descendants(
				component.element(),
				'.' + hConfig.itemClass()
			);
			var itemComps = cat(
		...
		var highlights = function() {
			var get = function(component) {
				return Highlighting.getHighlighted(component).map(function(
					item
				) {
					return item.element();
				});
		...
		var typical = function(
			...
			var processKey = function(
				...
				return choose$2(rules, simulatedEvent.event()).bind(function(
					rule$$1
				) {
					return rule$$1(
						component,
		...
		var create$2 = function(cyclicField) {
			...
			var findInitial = function(component, tabbingConfig) {
				var tabstops = $_bpsi2e119jkmcwtzh.descendants(
					component.element(),
			...
			var findCurrent = function(component, tabbingConfig) {
				return tabbingConfig
					...
					.get(component)
					.bind(function(elem) {
						return $_cbcewd11bjkmcwtzq.closest(
							elem,
							tabbingConfig.selector()
						);
					});
			...
			var go = function(component, simulatedEvent, tabbingConfig, cycle) {
				...
				return findCurrent(component, tabbingConfig).bind(function(
					...
					var optStopIndex = findIndex(
						tabstops,
						curry($_cpo6bcytjkmcwtkv.eq, tabstop)
					);
		...
		var inside = function(target) {
			return (
				($_b0ubstykjkmcwtjz.name(target) === 'input' &&
					$_ga5avvzwjkmcwtro.get(target, 'type') !== 'radio') ||
				$_b0ubstykjkmcwtjz.name(target) === 'textarea'
			);
		};
		...
		var getDirection = function(element) {
			return $_ezdyn3116jkmcwtyw.get(element, 'direction') === 'rtl'
				? 'rtl'
				: 'ltr';
		};
		...
		var locateIn = function(container, current, selector, filter$$1) {
			...
			var candidates = $_bpsi2e119jkmcwtzh.descendants(
				container,
				selector
			);
		...
		var cycleHorizontal = function(values, index, numRows, numCols, delta) {
			return withGrid(values, index, numCols, function(
				oldRow,
				oldColumn
			) {
		...
		var focusIn$2 = function(component, matrixConfig) {
			var focused = matrixConfig
				...
				.orThunk(function() {
					...
					return $_cbcewd11bjkmcwtzq.descendant(
						component.element(),
						selectors.cell()
					);
				});
		...
		var toMatrix = function(rows, matrixConfig) {
			return map$1(rows, function(row) {
				return $_bpsi2e119jkmcwtzh.descendants(
					row,
		...
		var doMove$2 = function(ifCycle, ifMove) {
			return function(element, focused, matrixConfig) {
				...
				return $_cbcewd11bjkmcwtzq
					...
					.bind(function(inRow) {
						var cellsInRow = $_bpsi2e119jkmcwtzh.descendants(
							inRow,
							matrixConfig.selectors().cell()
						);
						return findIndex$2(cellsInRow, focused).bind(function(
							colIndex
						) {
							var allRows = $_bpsi2e119jkmcwtzh.descendants(
								element,
								matrixConfig.selectors().row()
							);
							return findIndex$2(allRows, inRow).bind(function(
								rowIndex
							) {
								var matrix = toMatrix(allRows, matrixConfig);
								return move$$1(matrix, rowIndex, colIndex).map(
									function(next) {
										return next.cell();
									}
								);
				...
				function() {
					throw new Error(
						'Unknown placeholder component: ' +
							...
							owner.getOr('none') +
							'\nSpec: ' +
							$_ccklnrzhjkmcwton.stringify(compSpec, null, 2)
					);
				},
				function(newSpec) {
		...
		var read$2 = function(elem) {
			var id = $_b0ubstykjkmcwtjz.isElement(elem)
				? $_ga5avvzwjkmcwtro.get(elem, idAttr$1)
				: null;
		...
		var fromHtml$2 = function(html) {
			...
			var contents =
				children.length === 0
					? {}
					: { innerHtml: $_bu5gev10fjkmcwtu8.get(elem) };
			return deepMerge(
				{
		...
		var dom$1 = function(rawHtml) {
			var html = supplant(rawHtml, {
				prefix: $_dpmvs510tjkmcwtwz.prefix()
			});
		...
		var getToggleBehaviours = function(command) {
			return derive$2([
				Toggling.config({
					toggleClass: $_dpmvs510tjkmcwtwz.resolve(
		...
		var onLoad$1 = function(component, repConfig, repState) {
			repConfig
				.store()
				.manager()
				.onLoad(component, repConfig, repState);
		};
		...
		var sketch$1 = function(detail, components$$1, spec, externals) {
			...
			var changeValue = function(component, newValue) {
				...
				if (
					oldValue !== newValue ||
					$_ezdyn3116jkmcwtyw.getRaw(thumb.element(), 'left').isNone()
				) {
		...
		var button = function(realm, clazz, makeItems) {
			return $_3u2u4g10ujkmcwtx2.forToolbar(
				clazz,
				function() {
					var items = makeItems();
		...
		var makeSlider = function(spec$$1) {
			...
			return Slider.sketch({
				...
				sliderBehaviours: derive$2([
					$_46xi8a10sjkmcwtws.orientation(Slider.refresh)
				])
			});
		};
		...
		var getRawOrComputed = function(isRoot, rawStart) {
			var optStart = $_b0ubstykjkmcwtjz.isElement(rawStart)
				? Option.some(rawStart)
				: $_a7cj16ynjkmcwtk5.parent(rawStart);
			return optStart
				.map(function(start) {
					var inline = $_exwlcm103jkmcwtsk
						.closest(
							start,
							function(elem) {
								return $_ezdyn3116jkmcwtyw
						...
						.bind(function(elem) {
							return $_ezdyn3116jkmcwtyw.getRaw(
								elem,
								'font-size'
							);
						});
		...
		function blobToCanvas(blob) {
			return blobToImage(blob).then(function(image) {
				...
				var context, canvas;
				canvas = $_gg65x7139jkmcwueu.create(
					$_coszfc13ajkmcwuew.getWidth(image),
					$_coszfc13ajkmcwuew.getHeight(image)
				);
					...
					Representing.config({
						store: {
							...
							setValue: function(input, data) {
								var current = $_4w51n413tjkmcwuhq.get(
									input.element()
								);
								if (current !== data) {
									$_4w51n413tjkmcwuhq.set(
										input.element(),
										data
									);
								}
		...
		var hasNative = function(component) {
			return contains(
				nativeDisabled,
				$_b0ubstykjkmcwtjz.name(component.element())
			);
		};
		...
		var ariaIsDisabled = function(component) {
			return (
				$_ga5avvzwjkmcwtro.get(component.element(), 'aria-disabled') ===
				'true'
			);
		};
		...
		var sketch$7 = function(rawSpec) {
			...
			var navigationButton = function(direction, directionName, enabled) {
				return Button.sketch({
					...
					buttonBehaviours: derive$2([
						Disabling.config({
							disableClass: $_dpmvs510tjkmcwtwz.resolve(
			...
			var navigate = function(dialog, direction) {
				var screens = $_bpsi2e119jkmcwtzh.descendants(
					dialog.element(),
					'.' +
						$_dpmvs510tjkmcwtwz.resolve('serialised-dialog-screen')
				);
				$_cbcewd11bjkmcwtzq
					.descendant(
						dialog.element(),
			...
			var focusInput = function(dialog) {
				var inputs = $_bpsi2e119jkmcwtzh.descendants(
					dialog.element(),
					'input'
				);
				...
				Form.sketch(function(parts) {
					return {
						...
						formBehaviours: derive$2([
							$_46xi8a10sjkmcwtws.orientation(function(
								dialog,
								message
							) {
			...
			var memDots = record({
				...
				behaviours: derive$2([
					Highlighting.config({
						highlightClass: $_dpmvs510tjkmcwtwz.resolve(
							'dot-active'
						),
					...
					config(wrapperAdhocEvents, [
						...
						run(touchend(), function(wrapper) {
							spec$$1.state.dialogSwipeState.on(function(state) {
								...
								var direction =
									-1 * $_2173d7144jkmcwuk7.complete(state);
								navigate(dialog, direction);
							});
						})
					])
			...
			return [
				{
					...
					items: [
						sketch$7({
							...
							getInitialValue: function() {
								return Option.some(
									$_83q2l713hjkmcwufc.getInfo(editor)
								);
							},
		...
		var sketch$8 = function(realm, editor) {
			return $_3u2u4g10ujkmcwtx2.forToolbarStateAction(
				editor,
				'link',
				'link',
				function() {
		...
		var isRecursive = function(component, originator, target) {
			return (
				$_cpo6bcytjkmcwtkv.eq(originator, component.element()) &&
				!$_cpo6bcytjkmcwtkv.eq(originator, target)
			);
		};
		var events$8 = derive([
		...
		var combine$1 = function(info, baseMod, behaviours, base) {
			...
			var modifications = mapToArray(usedAspect, function(
				values$$1,
				aspect
			) {
					...
					if (aIndex === -1) {
						throw new Error(
							'The ordering for ' +
								...
								aKey +
								'.\nOrder specified: ' +
								$_ccklnrzhjkmcwton.stringify(order, null, 2)
						);
					}
					if (bIndex === -1) {
						throw new Error(
		...
		var onHover = function(item) {
			if (
				$_fci5qy101jkmcwtsc.search(item.element()).isNone() ||
		...
		var builder$2 = function(detail) {
			...
			var focusWidget = function(component) {
				return getPart(component, detail, 'widget').map(function(
					widget
				) {
		...
		var preserve$2 = function(f, container) {
			...
			var refocus = $_fci5qy101jkmcwtsc
				.active(ownerDoc)
				.bind(function(focused) {
					...
					return hasFocus(container)
						...
						: $_exwlcm103jkmcwtsk.descendant(container, hasFocus);
				});
		...
		var init$3 = function() {
			...
			var collapse = function(itemValue) {
				return readOptFrom$1(paths.get(), itemValue).bind(function(
					path
				) {
		...
		var make$3 = function(detail, rawUiSpec) {
			...
			var updateMenuPath = function(container, state, path) {
				return Option.from(path[0])
					...
					.map(function(activeMenu) {
						...
						$_c1ba8t14mjkmcwuq6.remove(activeMenu.element(), [
							detail.markers().backgroundMenu()
						]);
						setActiveMenu(container, activeMenu);
						var others = getMenus(state, state.otherMenus(path));
						each$1(others, function(o) {
							$_c1ba8t14mjkmcwuq6.remove(o.element(), [
								detail.markers().backgroundMenu()
							]);
							if (!detail.stayInDom()) {
				...
				return layeredState.expand(value).bind(function(path) {
					Option.from(path[0])
						...
						.each(function(activeMenu) {
							if (
								!$_3wo4fwyhjkmcwtje.inBody(activeMenu.element())
							) {
								Replacing.append(
									container,
			...
			var collapseMenuApi = function(container) {
				Highlighting.getHighlighted(container).each(function(
					currentMenu
				) {
					Highlighting.getHighlighted(currentMenu).each(function(
						currentItem
					) {
		...
		var getCurrentRoute = function(comp, transConfig, transState) {
			var el = comp.element();
			return $_ga5avvzwjkmcwtro.has(el, transConfig.destinationAttr())
				? Option.some({
						start: constant(
							$_ga5avvzwjkmcwtro.get(
								comp.element(),
		...
		var getState = function(comp, transConfig, transState) {
			var e = comp.element();
			return $_ga5avvzwjkmcwtro.has(e, transConfig.stateAttr())
				? Option.some(
						$_ga5avvzwjkmcwtro.get(e, transConfig.stateAttr())
				  )
				: Option.none();
		};
		...
		var makeItem = function(value, text$$1, selected, preview, isMenu) {
			return {
				...
				dom: {
					tag: 'div',
					classes: isMenu
						? [$_dpmvs510tjkmcwtwz.resolve('styles-item-is-menu')]
						: []
				},
				toggling: {
					toggleOnExecute: false,
				...
				itemBehaviours: derive$2(
					isMenu
						? []
						: [
								$_46xi8a10sjkmcwtws.format(value, function(
									comp,
									status
								) {
									var toggle = status
				...
				components: [
					Button.sketch({
						dom: {
							tag: 'div',
							classes: [
								$_dpmvs510tjkmcwtwz.resolve('styles-collapser')
							]
						},
						...
						behaviours: derive$2([
							config('adhoc-scrollable-menu', [
								runOnAttached(function(
									...
									$_9pks4l159jkmcwuvq.register(
										component.element()
									);
								}),
								runOnDetached(function(component) {
									$_ezdyn3116jkmcwtyw.remove(
										component.element(),
										'overflow-y'
									);
									$_ezdyn3116jkmcwtyw.remove(
			...
			var memMenu = record(
				tieredMenu.sketch({
					...
					onCollapseMenu: function(container, item, menu) {
						var submenu = $_cbcewd11bjkmcwtzq
							.ancestor(item.element(), '[role="menu"]')
							.getOrDie('hacky');
		...
		var setup = function(realm, editor) {
			...
			var stateCommandSketch = function(name) {
				return function() {
					return $_3u2u4g10ujkmcwtx2.forToolbarStateCommand(
						editor,
						name
					);
				};
			...
			var styleFormats = $_g500p1145jkmcwuk9.register(
				editor,
				editor.settings
			);
			...
			var styleselect = function() {
				return $_3u2u4g10ujkmcwtx2.forToolbar(
					...
					derive$2([
						...
						Receiving.config({
							channels: wrapAll$1([
								$_46xi8a10sjkmcwtws.receive(
									$_5zff6b109jkmcwtt8.orientationChanged(),
									Toggling.off
								),
								$_46xi8a10sjkmcwtws.receive(
									$_5zff6b109jkmcwtt8.dropupDismissed(),
									Toggling.off
								)
							])
						})
		...
		var capture$1 = function(element, event, handler) {
			return $_7vpx6n15djkmcwuwd.capture(
				element,
				event,
				filter$1,
				handler
		...
		var get$11 = function(outerWindow) {
			var isPortrait = outerWindow.matchMedia('(orientation: portrait)')
				.matches;
		...
		var onChange = function(outerWindow, listeners) {
			...
			var orientationHandle = $_csbuij15cjkmcwuwa.bind(
				win,
				'orientationchange',
				change
			);
		...
		var monitor$1 = function(editorApi) {
			...
			var onTouchend = function() {
				return $_csbuij15cjkmcwuwa.bind(
					editorApi.body(),
					'touchend',
					function(evt) {
		...
		var initEvents = function(editorApi, toolstrip, alloy) {
			...
			var isRanged = function(sel) {
				return (
					!$_cpo6bcytjkmcwtkv.eq(sel.start(), sel.finish()) ||
			...
			var listeners = [
				...
				tapping.onTouchmove(),
				tapping.onTouchend(),
				$_csbuij15cjkmcwuwa.bind(toolstrip, 'touchstart', function(
					evt
				) {
		...
		var safeParse = function(element, attribute) {
			var parsed = parseInt(
				$_ga5avvzwjkmcwtro.get(element, attribute),
				10
			);
		...
		var getEnd = function(element) {
			return $_b0ubstykjkmcwtjz.name(element) === 'img'
				? 1
				: $_br3bx415pjkmcwuz4.getOption(element).fold(
						function() {
		...
		var after$2 = function(start, soffset, finish, foffset) {
			var r = makeRange(start, soffset, finish, foffset);
			var same =
				$_cpo6bcytjkmcwtkv.eq(start, finish) && soffset === foffset;
		...
		var getRanges = function(win, selection) {
			return selection.match({
				...
				relative: function(startSitu, finishSitu) {
					return {
						ltr: cached(function() {
							return $_3a70ou15wjkmcwuzy.relativeToNative(
								win,
								startSitu,
								finishSitu
							);
						...
						rtl: cached(function() {
							return Option.some(
								$_3a70ou15wjkmcwuzy.relativeToNative(
									win,
									finishSitu,
									startSitu
								)
				...
				exact: function(start, soffset, finish, foffset) {
					return {
						ltr: cached(function() {
							return $_3a70ou15wjkmcwuzy.exactToNative(
								win,
								start,
								soffset,
								finish,
						...
						rtl: cached(function() {
							return Option.some(
								$_3a70ou15wjkmcwuzy.exactToNative(
									win,
									finish,
									foffset,
									start,
		...
		var locateOffset = function(doc, textnode, x, y, rect) {
			...
			var offset = $_4zooqd160jkmcwv0w.searchForPoint(
				rectForOffset,
				x,
				y,
				rect.right,
		...
		var locate$1 = function(doc, node, x, y) {
			...
			var foundRect = findMap(rects, function(rect) {
				return $_4zooqd160jkmcwv0w.inRect(rect, x, y)
					? Option.some(rect)
					: Option.none();
			});
		...
		var searchInChildren = function(doc, node, x, y) {
			...
			return findMap(nodes, function(n) {
				r.selectNode(n.dom());
				return $_4zooqd160jkmcwv0w.inRect(
		...
		var locateNode = function(doc, node, x, y) {
			var locator = $_b0ubstykjkmcwtjz.isText(node)
				? $_2cs89p161jkmcwv0y.locate
				: searchInChildren;
		...
		var first$3 = function(element) {
			return $_exwlcm103jkmcwtsk.descendant(
				element,
				$_bqeu8p15ojkmcwuz1.isCursorPosition
			);
		};
		...
		var locateInElement = function(doc, node, x) {
			...
			var f =
				collapseDirection === COLLAPSE_TO_LEFT
					? $_ct7f7e163jkmcwv17.first
					: $_ct7f7e163jkmcwv17.last;
		...
		var search$1 = function(doc, node, x) {
			var f =
				$_a7cj16ynjkmcwtk5.children(node).length === 0
					? locateInEmpty
					: locateInElement;
			return f(doc, node, x);
		};
		...
		var searchFromPoint = function(doc, x, y) {
			return Element$$1.fromPoint(doc, x, y).bind(function(elem) {
				...
				return $_a7cj16ynjkmcwtk5.children(elem).length === 0
					? fallback()
		...
		var fromPoint$1 = function(win, x, y) {
			...
			return availableSearch(doc, x, y).map(function(rng) {
				return $_9isfed15rjkmcwuzb.range(
		...
		var withinContainer = function(win, ancestor, outerRange, selector) {
			...
			var self = $_2lpyciyyjkmcwtlc.is(ancestor, selector)
				? [ancestor]
				: [];
			var elements = self.concat(
				$_bpsi2e119jkmcwtzh.descendants(ancestor, selector)
			);
		...
		var find$3 = function(win, selection, selector) {
			...
			return $_b0ubstykjkmcwtjz.isElement(ancestor)
		...
		var beforeSpecial = function(element, offset) {
			...
			if ('input' === name) return $_66srvk15sjkmcwuzg.after(element);
			else if (!contains(['br', 'img'], name))
				return $_66srvk15sjkmcwuzg.on(element, offset);
			else
				return offset === 0
					? $_66srvk15sjkmcwuzg.before(element)
					: $_66srvk15sjkmcwuzg.after(element);
		};
		var preprocessRelative = function(startSitu, finishSitu) {
			var start = startSitu.fold(
				$_66srvk15sjkmcwuzg.before,
				beforeSpecial,
				$_66srvk15sjkmcwuzg.after
			);
		...
		var doSetRange = function(win, start, soffset, finish, foffset) {
			var rng = $_3a70ou15wjkmcwuzy.exactToNative(
				win,
				start,
				soffset,
				finish,
		...
		var setExact = function(win, start, soffset, finish, foffset) {
			var relative = $_71tigc165jkmcwv1f.preprocessExact(
				start,
				soffset,
				finish,
				foffset
		...
		var setRelative = function(win, startSitu, finishSitu) {
			var relative = $_71tigc165jkmcwv1f.preprocessRelative(
				startSitu,
				finishSitu
			);
			setRangeFromRelative(win, relative);
		...
		var doGetExact = function(selection) {
			...
			return $_gdadtz15ujkmcwuzt.after(
				anchorNode,
		...
		var get$13 = function(win) {
			return getExact(win).map(function(range) {
				return $_9isfed15rjkmcwuzb.exact(
					range.start(),
					range.soffset(),
					range.finish(),
					range.foffset()
		...
		var replace = function(win, selection, elements) {
			var rng = $_adttxu15xjkmcwv07.asLtrRange(win, selection);
			var fragment = $_daatfv15vjkmcwuzu.fromElements(
				elements,
				win.document
			);
		...
		var getRectsFromRange = function(range$$1) {
			if (!range$$1.collapsed) {
				...
				return $_a7cj16ynjkmcwtk5
					.parent(start_1)
					.bind(function(parent) {
						var selection = $_9isfed15rjkmcwuzb.exact(
							start_1,
							range$$1.startOffset,
							parent,
							$_bqeu8p15ojkmcwuz1.getEnd(parent)
						);
						var optRect = $_1ii7ra15tjkmcwuzm.getFirstRect(
		...
		var setup$1 = function(outerWindow, cWin) {
			...
			var onResize = $_csbuij15cjkmcwuwa.bind(
		...
		var tag = function() {
			...
			var element = $_cbcewd11bjkmcwtzq
		...
		var create$5 = function(platform, mask) {
			...
			var enter = function() {
				...
				$_4fq2lwzujkmcwtrk.add(
					platform.container,
					$_dpmvs510tjkmcwtwz.resolve('fullscreen-maximized')
				);
				$_4fq2lwzujkmcwtrk.add(
					platform.container,
					$_dpmvs510tjkmcwtwz.resolve('android-maximized')
				);
				meta.maximize();
				$_4fq2lwzujkmcwtrk.add(
					platform.body,
					$_dpmvs510tjkmcwtwz.resolve('android-scroll-reload')
				);
				androidApi.set(
					$_es65ig15ljkmcwuy8.setup(
						platform.win,
						$_46e9af167jkmcwv1t
				...
				$_46e9af167jkmcwv1t
					...
					.each(function(editorApi) {
						$_2a80we168jkmcwv26.clobberStyles(
							platform.container,
							editorApi.body()
						);
						androidEvents.set(
							$_4zjqxd15hjkmcwux6.initEvents(
								editorApi,
								platform.toolstrip,
								platform.alloy
							)
		...
		var produce = function(raw) {
			...
			var mask = build$1(
				$_5eqauz16ajkmcwv2t.sketch(onTap, mobile.translate)
			);
		...
		var dataHorizontal =
			'data-' + $_dpmvs510tjkmcwtwz.resolve('horizontal-scroll');
		...
		var hasScroll = function(container) {
			return $_ga5avvzwjkmcwtro.get(container, dataHorizontal) === 'true'
				? hasHorizontalScroll(container)
				: hasVerticalScroll(container);
		};
		var exclusive = function(scope, selector) {
			return $_csbuij15cjkmcwuwa.bind(scope, 'touchmove', function(
				event
			) {
				$_cbcewd11bjkmcwtzq
		...
		function ScrollingToolbar() {
			var makeGroup = function(gSpec) {
				...
				return {
							...
							gSpec.scrollable === true
								? [
										runOnInit(function(
											...
											$_6uc3h316ijkmcwv51.markAsHorizontal(
												component.element()
											);
											$_9pks4l159jkmcwuvq.register(
												component.element()
					...
					markers: {
						itemClass: $_dpmvs510tjkmcwtwz.resolve(
							'toolbar-group-item'
						)
					},
					items: gSpec.items
			...
			var wrapper = build$1(
				Container.sketch({
					dom: {
						classes: [$_dpmvs510tjkmcwtwz.resolve('toolstrip')]
					},
		...
		var isGrowing = function(component, slideConfig, slideState) {
			var root = getAnimationRoot(component, slideConfig);
			return (
				$_4fq2lwzujkmcwtrk.has(root, slideConfig.growingClass()) ===
				true
			);
		};
		var isShrinking = function(component, slideConfig, slideState) {
			var root = getAnimationRoot(component, slideConfig);
			return (
				$_4fq2lwzujkmcwtrk.has(root, slideConfig.shrinkingClass()) ===
				true
			);
		};
			...
			var dropup = build$1(
				Container.sketch({
					...
					containerBehaviours: derive$2([
						Replacing.config({}),
						Sliding.config({
							closedClass: $_dpmvs510tjkmcwtwz.resolve(
								'dropup-closed'
							),
							openClass: $_dpmvs510tjkmcwtwz.resolve(
								'dropup-open'
							),
							shrinkingClass: $_dpmvs510tjkmcwtwz.resolve(
								'dropup-shrinking'
							),
							growingClass: $_dpmvs510tjkmcwtwz.resolve(
								'dropup-growing'
							),
		...
		var setup$2 = function(container, rawSettings) {
			...
			var simpleEvents = map$1(
				...
				function(type) {
					return $_csbuij15cjkmcwuwa.bind(container, type, function(
						event$$1
					) {
						tapEvent
			...
			var onKeydown = $_csbuij15cjkmcwuwa.bind(
				container,
				'keydown',
				function(event$$1) {
			...
			var onWindowScroll = $_csbuij15cjkmcwuwa.bind(
				defaultView,
				'scroll',
				function(event$$1) {
		...
		var closest$3 = function(target, transform, isRoot) {
			var delegate = $_exwlcm103jkmcwtsk.closest(
				target,
				function(elem) {
				...
				if (conflict === component) {
					...
					throw new Error(
						'The tagId "' +
							...
							($_3wo4fwyhjkmcwtje.inBody(conflict.element())
								? ' '
								: ' not ') +
							'already in the DOM'
					);
		...
		var takeover = function(root) {
			var isAboveRoot = function(el) {
				return $_a7cj16ynjkmcwtk5.parent(root.element()).fold(
					function() {
						return true;
					},
					function(parent) {
		...
		var READ_ONLY_MODE_CLASS = constant(
			$_dpmvs510tjkmcwtwz.resolve('readonly-mode')
		);
		var EDIT_MODE_CLASS = constant(
			$_dpmvs510tjkmcwtwz.resolve('edit-mode')
		);
		...
		function OuterContainer(spec) {
			var root = build$1(
				Container.sketch({
					dom: {
						classes: [
							$_dpmvs510tjkmcwtwz.resolve('outer-container')
						].concat(spec.classes)
					},
					containerBehaviours: derive$2([
						Swapping.config({
		...
		function AndroidRealm(scrollIntoView) {
			var alloy = OuterContainer({
				classes: [$_dpmvs510tjkmcwtwz.resolve('android-container')]
			});
		...
		var stubborn = function(outerBody, cWin, page, frame) {
			...
			var captureInput = $_csbuij15cjkmcwuwa.bind(
				page,
				'keydown',
				function(evt) {
					if (
		...
		var initEvents$1 = function(
			...
			var listeners = [
				...
				$_csbuij15cjkmcwuwa.bind(editorApi.doc(), 'touchend', function(
					touchEvent
				) {
					if (
						$_cpo6bcytjkmcwtkv.eq(
							editorApi.html(),
							touchEvent.target()
						) ||
						$_cpo6bcytjkmcwtkv.eq(
				...
				$_csbuij15cjkmcwuwa.bind(toolstrip, 'transitionend', function(
					transitionEvent
				) {
				...
				$_csbuij15cjkmcwuwa.capture(toolstrip, 'touchstart', function(
					touchEvent
				) {
					saveSelectionFirst();
				...
				tapping.onTouchmove(),
				tapping.onTouchend(),
				$_csbuij15cjkmcwuwa.bind(editorApi.body(), 'click', function(
					event
				) {
					event.kill();
				}),
		...
		function FakeSelection(win, frame) {
			...
			$_4fq2lwzujkmcwtrk.add(
				container,
				$_dpmvs510tjkmcwtwz.resolve('unfocused-selections')
			);
			$_1qcoujymjkmcwtk3.append(
				Element$$1.fromDom(doc.documentElement),
				container
			);
			var onTouch = $_csbuij15cjkmcwuwa.bind(
				container,
				'touchstart',
				function(event) {
					event.prevent();
		...
		var softKeyboardLimits = function(outerWindow) {
			return $_cvf14s17gjkmcwve3.findDevice(
				outerWindow.screen.width,
				outerWindow.screen.height
			);
		};
		...
		var getGreenzone = function(socket, dropup) {
			var outerWindow = $_a7cj16ynjkmcwtk5.owner(socket).dom()
				.defaultView;
			var viewportHeight =
				$_3mqbs115jkmcwtyu.get(socket) + $_3mqbs115jkmcwtyu.get(dropup);
		...
		var updatePadding = function(contentBody, socket, dropup) {
			...
			var deltaHeight =
				$_3mqbs115jkmcwtyu.get(socket) +
				$_3mqbs115jkmcwtyu.get(dropup) -
				greenzoneHeight;
			$_ezdyn3116jkmcwtyw.set(
				contentBody,
				'padding-bottom',
		...
		var yFixedData =
			'data-' + $_dpmvs510tjkmcwtwz.resolve('position-y-fixed');
		var yFixedProperty =
			'data-' + $_dpmvs510tjkmcwtwz.resolve('y-property');
		var yScrollingData = 'data-' + $_dpmvs510tjkmcwtwz.resolve('scrolling');
		var windowSizeData =
			'data-' + $_dpmvs510tjkmcwtwz.resolve('last-window-height');
		...
		var classify = function(element) {
			...
			var classifier =
				$_ga5avvzwjkmcwtro.get(element, yScrollingData) === 'true'
					? classifyScrolling
					: classifyFixed;
		...
		var deriveViewportHeight = function(
			...
			var outerWindow = $_a7cj16ynjkmcwtk5.owner(viewport).dom()
				.defaultView;
		...
		var takeover$1 = function(viewport, contentBody, toolbar, dropup) {
			...
			var refresh = function() {
				if (isActive) {
					...
					$_5n5el317fjkmcwvdx.updatePadding(
						contentBody,
						viewport,
						dropup
					);
		...
		var lastScroll =
			'data-' + $_dpmvs510tjkmcwtwz.resolve('last-scroll-top');
		...
		var moveOnlyScroll = function(element, destination) {
			return Future.nu(function(callback) {
				...
				var update = function(newScroll, abort) {
					var previous = $_1qtglj15mjkmcwuyi.safeParse(
						element,
						lastScroll
					);
		...
		var updateTop = function(element, amount) {
			var newTop =
				amount + $_exi4dz17ejkmcwvdb.getYFixedData(element) + 'px';
			$_ezdyn3116jkmcwtyw.set(element, 'top', newTop);
		};
		var moveWindowScroll = function(toolbar, viewport, destY) {
			var outerWindow = $_a7cj16ynjkmcwtk5.owner(toolbar).dom()
				.defaultView;
			return Future.nu(function(callback) {
		...
		var register$2 = function(
			...
			var scroller = BackgroundActivity(function(y) {
				return $_dl3ja8179jkmcwvcs.moveWindowScroll(
					toolstrip,
					socket,
					y
				);
			...
			var onScroll = $_csbuij15cjkmcwuwa.bind(
				Element$$1.fromDom(outerWindow),
				'scroll',
				function() {
		...
		var setup$3 = function(bag) {
			...
			var structure = $_exi4dz17ejkmcwvdb.takeover(
				socket,
				ceBody,
				toolstrip,
				dropup
			...
			var scrollIntoView = function(top, bottom) {
				$_59bkbi17ijkmcwvec.scrollIntoView(
					cWin,
					socket,
					dropup,
					top,
			...
			var destroy = function() {
				...
				$_7gxy0a173jkmcwvas.input(
					$_3wo4fwyhjkmcwtje.body(),
					$_fci5qy101jkmcwtsc.blur
				);
			};
			return {
				toEditing: toEditing,
		...
		var create$8 = function(platform, mask) {
			...
			var enter = function() {
				...
				$_46e9af167jkmcwv1t
					...
					.each(function(editorApi) {
						priorState.set({
							socketHeight: $_ezdyn3116jkmcwtyw.getRaw(
								platform.socket,
								'height'
							),
							iframeHeight: $_ezdyn3116jkmcwtyw.getRaw(
								editorApi.frame(),
								'height'
							),
						...
						scrollEvents.set({
							exclusives: $_6uc3h316ijkmcwv51.exclusive(
								doc,
								'.' + $_9pks4l159jkmcwuvq.scrollable()
							)
						});
						$_4fq2lwzujkmcwtrk.add(
							platform.container,
		...
		var produce$1 = function(raw) {
			...
			var mask = build$1(
				$_5eqauz16ajkmcwv2t.sketch(onView, mobile.translate)
			);
			mobile.alloy.add(mask);
			var maskApi = {
				show: function() {
		...
		function IosRealm(scrollIntoView) {
			var alloy = OuterContainer({
				classes: [$_dpmvs510tjkmcwtwz.resolve('ios-container')]
			});
			var toolbar = ScrollingToolbar();
			var webapp = api$2();
			var switchToEdit = $_51jxa16jjkmcwv5a.makeEditSwitch(webapp);
		...
		var init$5 = function(realm, editor) {
			...
			each$1(['ul', 'ol'], function(command) {
				editor.selection.selectorChanged(command, function(
					state,
					data
				) {
					fireChange$1(realm, command, state);
		...
		global$1.add('mobile', function(editor) {
			var renderUI = function(args) {
				...
				var realm = $_b90373y3jkmcwths.detect().os.isAndroid()
				...
				var orientation = $_c3oqmx15bjkmcwuw0.onChange(outerWindow, {
					onChange: function() {
						...
						alloy.broadcastOn(
							[$_5zff6b109jkmcwtt8.orientationChanged()],
							{
								width: $_c3oqmx15bjkmcwuw0.getActualWidth(
									outerWindow
								)
							}
						);
				...
				editor.on('init', function() {
					realm.init({
						editor: {
							...
							onScrollToCursor: function(handler) {
								editor.on('scrollIntoView', function(
									tinyEvent
								) {
									handler(tinyEvent);
								});
							...
							onTapContent: function(evt) {
								...
								if ($_b0ubstykjkmcwtjz.name(target) === 'img') {
									...
									$_b0ubstykjkmcwtjz.name(target) === 'a'
								) {
									var component = realm
										.system()
										.getByDom(
						...
						toolstrip: Element$$1.fromDom(
							editor.editorContainer.querySelector(
								'.' + $_dpmvs510tjkmcwtwz.resolve('toolstrip')
							)
						),
						toolbar: Element$$1.fromDom(
							editor.editorContainer.querySelector(
								'.' + $_dpmvs510tjkmcwtwz.resolve('toolbar')
							)
						),
						dropup: realm.dropup(),
						alloy: realm.system(),
					...
					var items = $_1rc1u10ajkmcwttb.detect(
						editor.settings,
						features
					);
					var actionGroup = {
