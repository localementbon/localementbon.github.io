(function() {
	var inlite = (function() {
		...
		var getSelectionRect = function(editor) {
			...
			return clientRect
				? toAbsolute($_cs3afq181jkmcwwpy.fromClientRect(clientRect))
				: null;
		};
			...
			return function(editor) {
				for (var i = 0; i < predicateIds.length; i++) {
					if (predicateIds[i].predicate(element)) {
						var result = $_bv44i317zjkmcwwpv.result(
							predicateIds[i].id,
							$_2fb1t1180jkmcwwpw.getElementRect(editor, element)
						);
						return result;
					}
				}
				...
				for (var i = 0; i < elements.length; i++) {
					for (var x = 0; x < predicateIds.length; x++) {
						if (predicateIds[x].predicate(elements[i])) {
							return $_bv44i317zjkmcwwpv.result(
								predicateIds[x].id,
								$_2fb1t1180jkmcwwpw.getElementRect(
									editor,
									elements[i]
								)
							);
		...
		var textSelection = function(id) {
			return function(editor) {
				if (!editor.selection.isCollapsed()) {
					var result = $_bv44i317zjkmcwwpv.result(
						id,
						$_2fb1t1180jkmcwwpw.getSelectionRect(editor)
					);
					return result;
				}
				return null;
		...
		var userConstrain = function(
			...
			if (typeof handler === 'function') {
				userConstrainedPanelRect = handler({
					...
					contentAreaRect: $_cs3afq181jkmcwwpy.toClientRect(
						contentAreaRect
					),
					panelRect: $_cs3afq181jkmcwwpy.toClientRect(panelRect)
				});
				return $_cs3afq181jkmcwwpy.fromClientRect(
		...
		var getTextSelectionToolbarItems = function(editor) {
			return $_eygqjy189jkmcwwq6.getToolbarItemsOr(
				editor,
				'selection_toolbar',
		...
		var getPositionHandler = function(editor) {
			return $_eygqjy189jkmcwwq6.getHandlerOr(
				editor,
				'inline_toolbar_position_handler',
				$_blqb9d18bjkmcwwq9.defaultHandler
			);
		};
		...
		var overrideLinkShortcut = function(editor, panel) {
			...
			editor.shortcuts.add('meta+k', '', function() {
				var toolbars = getToolbars(editor);
				var result = $_bv44i317zjkmcwwpv.match(editor, [
					$_gio032184jkmcwwq0.textSelection('quicklink')
				]);
				if (result) {
					panel.show(editor, result.id, result.rect, toolbars);
				}
		...
		var proto$1 = {
			...
			init: function(settings) {
				...
				self$$1.borderBox = $_4qv1hp18pjkmcwws9.parseBox(
					settings.border
				);
				self$$1.paddingBox = $_4qv1hp18pjkmcwws9.parseBox(
					settings.padding
				);
				self$$1.marginBox = $_4qv1hp18pjkmcwws9.parseBox(
					settings.margin
				);
				if (settings.hidden) {
					self$$1.hide();
			...
			initLayoutRect: function() {
				...
				borderBox = self$$1.borderBox =
					self$$1.borderBox ||
					$_4qv1hp18pjkmcwws9.measureBox(elm, 'border');
				self$$1.paddingBox =
					self$$1.paddingBox ||
					$_4qv1hp18pjkmcwws9.measureBox(elm, 'padding');
				self$$1.marginBox =
					self$$1.marginBox ||
					$_4qv1hp18pjkmcwws9.measureBox(elm, 'margin');
		...
		function calculateRelativePosition(ctrl, targetElm, rel) {
			...
			pos = funcs.getPos(
				targetElm,
				$_5d7kox18xjkmcwwss.getUiContainer(ctrl)
			);
			x = pos.x;
			y = pos.y;
		...
		var FloatPanel = Panel.extend({
			Mixins: [$_928of518yjkmcwwsw, $_29brjy19bjkmcwwuo],
			init: function(settings) {
				var self$$1 = this;
		...
		var convertLinkToAbsolute = function(editor, href) {
			return !$_bo936019ujkmcwww5.isAbsolute(href) &&
				$_bo936019ujkmcwww5.isDomainLike(href)
		...
		var createQuickLinkForm = function(editor, hide) {
			...
			return createForm('quicklink', {
				...
				onsubmit: function(e) {
					convertLinkToAbsolute(editor, e.data.linkurl).then(function(
						url
					) {
						editor.undoManager.transact(function() {
		...
		var create$3 = function() {
			...
			var createToolbars = function(editor, toolbars) {
				return global$4.map(toolbars, function(toolbar) {
					return $_9njq9c19vjkmcwww6.create(
						editor,
						toolbar.id,
						toolbar.items
					);
			...
			var create = function(editor, toolbars) {
				var items = createToolbars(editor, toolbars).concat([
					$_9njq9c19vjkmcwww6.create(
						editor,
						'text',
						$_3i7b7u187jkmcwwq4.getTextSelectionToolbarItems(editor)
					),
					$_9njq9c19vjkmcwww6.create(
						editor,
						'insert',
						$_3i7b7u187jkmcwwq4.getInsertToolbarItems(editor)
					),
			...
			var repositionPanelAt = function(panel, id, editor, targetRect) {
				...
				userConstainHandler = $_3i7b7u187jkmcwwq4.getPositionHandler(
					editor
				);
				contentAreaRect = $_2fb1t1180jkmcwwpw.getContentAreaRect(
					editor
				);
				panelRect = global$2.DOM.getRect(panel.getEl());
				if (id === 'insert') {
					result = $_blqb9d18bjkmcwwq9.calcInsert(
						targetRect,
						contentAreaRect,
						panelRect
					);
				} else {
					result = $_blqb9d18bjkmcwwq9.calc(
						targetRect,
						contentAreaRect,
						panelRect
					);
				...
				if (result) {
					...
					movePanelTo(
						panel,
						$_blqb9d18bjkmcwwq9.userConstrain(
							userConstainHandler,
							targetRect,
							contentAreaRect,
							transposedPanelRect
		...
		var isComment = function(element) {
			return (
				type(element) === $_110sa81apjkmcwwze.COMMENT ||
		...
		var inBody = function(element) {
			var dom = $_alzn281aojkmcwwzc.isText(element)
		...
		var detectBrowser = function(browsers, userAgent) {
			return detect$1(browsers, userAgent).map(function(browser) {
				var version = $_g9yxa11b2jkmcwx0y.detect(
		...
		var detect$2 = function(userAgent) {
			...
			var browser = $_5xz6bk1b5jkmcwx1f
				.detectBrowser(browsers, userAgent)
				.fold($_bgnekf1b1jkmcwx0u.unknown, $_bgnekf1b1jkmcwx0u.nu);
			var os = $_5xz6bk1b5jkmcwx1f
				.detectOs(oses, userAgent)
				.fold($_gq0pn1b3jkmcwx10.unknown, $_gq0pn1b3jkmcwx10.nu);
		...
		var ieContains = function(e1, e2) {
			return $_ay9tgw1ayjkmcwx0p.documentPositionContainedBy(
				e1.dom(),
				e2.dom()
			);
		};
		...
		var ancestors = function(scope, predicate, isRoot) {
			return filter(
				$_dw6n5t1aqjkmcwwzh.parents(scope, isRoot),
				predicate
			);
		};
		...
		var ancestors$1 = function(scope, selector, isRoot) {
			return $_aet82i1aljkmcwwz4.ancestors(
				scope,
				function(e) {
		...
		var select = function(selector, root) {
			return map(
				$_3g4jt21akjkmcwwz3.descendants(
		...
		var ancestor$1 = function(scope, selector, isRoot) {
			return $_5x60le1bgjkmcwx2f.ancestor(
				scope,
				function(e) {
					return $_6woo5p1bajkmcwx1u.is(e, selector);
				},
		...
		var register = function(editor) {
			...
			editor.addButton('align', {
				...
				onPostRender: function(e) {
					var ctrl = e.control;
					global$4.each(alignFormats, function(formatName, idx) {
						addFormatChangedListener(editor, formatName, function(
							state
						) {
		...
		var trimMenuItems = function(menuItems) {
			var menuItems2 = filter(menuItems, function(
				menuItem,
				i,
				menuItems
			) {
		...
		var setupUiContainer = function(editor) {
			if (editor.settings.ui_container) {
				global$1.container = $_1srzk21bfjkmcwx2d
					.descendant(
